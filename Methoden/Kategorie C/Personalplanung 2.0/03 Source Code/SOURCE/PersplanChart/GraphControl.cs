...
namespace UserControls{
	...
	public class GraphControl : System.Windows.Forms.UserControl
	{
		...
		ArrayList Series;
		ArrayList SeriesColor;
		ArrayList SeriesStyle;
		ArrayList SeriesName;
		...
		public void AddPointsSeries(int x, int y, int Series)
		{
			while (this.Series.Count < Series)
				this.Series.Add(new ArrayList());
			((ArrayList)this.Series[Series-1]).Add(new Point(x, y));
		}
		...
		public void ClearPointsSeries (int Series)
		{
			if (this.Series.Count >= Series && this.Series[Series-1] != null)
				((ArrayList)this.Series[Series-1]).Clear();
		}

		public void SetSeriesStyle (int Series, bool IsBar)
		{
			while (this.SeriesStyle.Count < Series)
				this.SeriesStyle.Add(false);
			this.SeriesStyle[Series-1] = IsBar;
		}

		public void SetSeriesColor(int Series, Color PenColor)
		{
			while (this.SeriesColor.Count < Series)			
				this.SeriesColor.Add(null);
			this.SeriesColor[Series-1] = PenColor;
		}
		
		public void SetSeriesName(int Series, string Name)
		{
			while (this.SeriesName.Count < Series)			
				this.SeriesName.Add("");
			this.SeriesName[Series-1] = Name;
		}
		...
		//draws the actual graph, multiple series. Each series as lines or bar.
		void DrawSeriesGraph(Graphics g)
		{
			int counter = 0;
			int NamesXPos = 0;
			
			foreach (ArrayList Points in this.Series)
			{
				if (Points.Count > 0)
				{
					Pen p = new Pen((Color)this.SeriesColor[counter]);
					
					// draw the name of the series
					string SeriesName = (string)this.SeriesName[counter];
					Brush br = new SolidBrush((Color)this.SeriesColor[counter]);
					Font fo = new Font(Font.FontFamily, Font.Size);
					g.DrawString(SeriesName, fo, br, origin.X + NamesXPos, origin.Y+15);
					NamesXPos += ((int)g.MeasureString(SeriesName, fo).Width) + 20;

					if (!(bool)this.SeriesStyle[counter])
					{
						// line style
		...
		void DrawTitles(Graphics g)
		{
			Brush b = new SolidBrush(textColor);
			Font f = new Font(Font.FontFamily,Font.Size);
			string title = "X-Achse = " + xTitle + "    " + "Y-Achse = " + yTitle;
