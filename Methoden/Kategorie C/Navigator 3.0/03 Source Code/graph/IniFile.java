...
public class IniFile extends Object
{
   ...
   */
   public void saveFile()
   {
      try
      {
         ...
         for (int i=0;i<lines.size();i++)
         {
            outFile.writeBytes((String)(lines.elementAt(i))+System.getProperty("line.separator"));
         }
         outFile.close();
      }
      catch (IOException e)
   ...
   */
   ...
   public static void main(String[] args)
   {
      if (args.length != 1)
      {
         System.out.println("IniFile <filename>");
         System.exit(0);
      }
      else
      {
         String[] vars;
         IniFile f = new IniFile(args[0]);
         String[] subs = f.getSubjects();
         for (int i = 0;i<subs.length;i++)
         {
            System.out.println(subs[i]);
            vars = f.getVariables(subs[i]);
            for (int j = 0;j<vars.length;j++)
            {
               System.out.println("  "+vars[j]+" = "+f.getValue(subs[i],vars[j]));
            }
         }
      }
   }
