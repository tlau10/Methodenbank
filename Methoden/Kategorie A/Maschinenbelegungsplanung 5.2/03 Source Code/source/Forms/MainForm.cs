...
namespace Maschinenbelegungsplanung
{
	public class Form1 : System.Windows.Forms.Form
	{
        ...
        //Solverpfad wird nun über Settings.settings Datei eingelesen
        public static string solverPathLP = Properties.Settings.Default.SolverPfadLP;

        public static string solverPathGLPK = Properties.Settings.Default.SolverPfadGLPK;
        
        //----------------------------------------------------------------------------------
        //Arbeitsverzeichnis wird nun über Settings.settings Datei eingelesen
        public static string workingDir = Properties.Settings.Default.Arbeitsverzeichnis;
        //----------------------------------------------------------------------------------
        ...
        static public PrintableAuftraegeDataGrid printableAuftraegeDataGridOM;
        static public PrintablePeriodenDataGrid printablePeriodenDataGridOM;
		...
		protected void CreateViewMenu(MenuCommand mc)
		{
            ...
            //Änderung im Rahmen der ANLO Veranstaltung. Graphische Ausgabeoption verfügbar 
			MenuCommand viewSplitterOne = new MenuCommand("-");
			MenuCommand viewGraphical = new MenuCommand("&Graphisch");
            viewGraphical.Enabled = true; 
            viewGraphical.Checked = true;
		...
		protected void CreatePreferencesMenu(MenuCommand mc)
		{
			...
			MenuCommand prefSolverLP = new MenuCommand("Solverpfad LP-Solve", new EventHandler( onSolverPathChangedLP ));
            MenuCommand prefSolverGLPK = new MenuCommand("Solverpfad GLPK", new EventHandler( onSolverPathChangedGLPK));
			//MenuCommand prefFunction = new MenuCommand("Zielfunktion");

			// Setup event handlers
            mc.MenuCommands.AddRange(new MenuCommand[] { prefFolder, prefSolverLP, prefSolverGLPK });
		...
		{
            //Im Rhamen der ANLO Vorlesung: Arbeitsverzeichnis wird nun über Einstellungs.xml Datei eingelesen 
		...
		protected void onSolverPathChangedLP( object sender, EventArgs e )
		{
            //Im Rhamen der ANLO Vorlesung: Solverpfad wird nun über Einstellungs.xml Datei eingelesen   
			PreferencesForm aPreferencesForm = new PreferencesForm( " Bitte geben Sie den Solverpfad ein: \n\n( Bspl: \" C:\\Methodendatenbank\\SOLVER\\LP_SOLVE\\EXEC \" ) ", solverPathLP , "solverPathLP" );
			aPreferencesForm.Show( );
		}

        protected void onSolverPathChangedGLPK(object sender, EventArgs e)
        {
            //Im Rhamen der ANLO Vorlesung: Solverpfad wird nun über Einstellungs.xml Datei eingelesen   
            PreferencesForm aPreferencesForm = new PreferencesForm(" Bitte geben Sie den Solverpfad ein: \n\n( Bspl: \" C:\\Solver\\glpk-4.55\\w64 \" ) ", solverPathGLPK, "solverPathGLPK");
            aPreferencesForm.Show();
        }
		...
		protected void onFileOpenClick( object sender, EventArgs e)
		{
			...
			openFileDialog1.InitialDirectory = "C:\\Methodenbank\\Methoden\\Kategorie A\\Maschinenbelegungsplanung 5.2\\01 Programm neu\\daten";
		...
		protected void onFileSaveAsClick( object sender, EventArgs e)
		{
			...
			if( result == DialogResult.OK )
			{
				...
				if (tabControl1.SelectedIndex == 0)
				{
					printableDataGridToEingabeDaten pDTE = new printableDataGridToEingabeDaten( printableAuftraegeDataGridOM( ), printablePeriodenDataGridOM( ) );
		...
		protected void onCalculateClicked( object sender, EventArgs e)
		{
			...
			if (tabControl1.SelectedIndex == 0)
			{
				pDTE = new printableDataGridToEingabeDaten( printableAuftraegeDataGridOM( ), printablePeriodenDataGridOM( ) );
				eingabeDaten data = pDTE.returnEingabeDaten(1);

				if( data.auftragsListe[0] != null )
				{
					ISolver theSolver = new OneMachineSolver( data, InputForm.solver, solverPathLP, solverPathGLPK, workingDir );
					theSolver.createMPS( );
					theSolver.solve( );
			...
			{   // Berechnung für mehrere Maschinen
				if (tabControl1.SelectedIndex == 1)
				{
					...
					if( data.auftragsListe[0] != null )
					{
                        ISolver theSolver = new MultipleMachinesSolver(data, InputForm.solver, solverPathLP, solverPathGLPK, workingDir, anzahlMaschinen);
						theSolver.createMPS( );
						theSolver.solve( );
						calculatedResults = theSolver.readResults( );
		...
		private void InitializeComponent()
		{
            ...
            this.tabControl1.Size = new System.Drawing.Size(792, 548);
            ...
            this.tabbedGroups1.Size = new System.Drawing.Size(792, 523);
            ...
            // tabPage2
            // 
            this.tabPage2.Controls.Add(this.tabbedGroups2);
            ...
            this.tabbedGroups2.Location = new System.Drawing.Point(0, 0);
            ...
            this.tabbedGroups2.Size = new System.Drawing.Size(792, 523);
