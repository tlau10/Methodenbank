...
public class OptiControlPanel extends JPanel {
	...
	private Object[] resultColumns = { "Lösung", "Datum", "Uhrzeit", "Gesamtzeit", "Durchschnittszeit" };
	...
	private List<TargetDetails> targets = null;
	...
	private OptiControlPanel() {
		...
		this.setBounds(25 + config.getResPanel(), 15, 100, config.getResPanel());

		configPanel = new JPanel();
		configPanel.setLayout(null);
		configPanel.setBorder(BorderFactory.createTitledBorder("Einstellungen"));
		...
		solutionInfoPanel.setBorder(BorderFactory.createTitledBorder("Ergebnis-Informationen"));
		...
		bStarten.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (netz.isEmpty() == false) {
					...
					if (goOn) {
						...
						final Thread t = new Thread(new Runnable() {
							...
							public void run() {
								...
								if (goOn) {
									if (optimierungRun == true && simulationStopped == false) {
										optimierungRun = false;
										createResultTable();
									}

									if (SimuKonfiguration.getInstance().checkSound()) {
										// make a signal after the optimizing
										File audioFile = new File(
												System.getProperty("user.dir") + "/sound/bus_horn.wav");
										try {
									...
									if (simulationStopped == false) {
										...
										JOptionPane.showMessageDialog(View.getInstance(),
												...
												// "Info",
												// JOptionPane.INFORMATION_MESSAGE);
												"Benötigte Zeit für die Berechnung: " + du.getTimeDiffAsString() + "\n",
												"Dauer von Operation Research", JOptionPane.INFORMATION_MESSAGE);
									} else {
						...
						stopButton.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent e) {
								if (optimierungInterrupted == true && JOptionPane.showOptionDialog(progressFrame,
										"Wollen Sie Sie die Optimierung abbrechen?", "Abbrechen",
					...
					JOptionPane.showMessageDialog(View.getInstance(),
							"Für die Optimierung fehlt entweder ein Ziel oder eine Strasse", "Bus Simulation",
		...
		progressFrame.setIconImage(new ToolkitImage(new URLImageSource(ImageUtil.getImageUrl("haltestelle.png"))));
	...
	public final class StartOptimierung extends Thread {
		...
		public void run() {
			...
			try {
				...
				columnWidth = new ColumnProperties(columns, columnWidths, resizables);
				progressBar.setValue(0);
				detailProgressBar.setValue(0);

				TargetBuilder targetBuilder = new TargetBuilder((Integer) maxComboBox.getSelectedItem());
				targets = targetBuilder.buildTargetDetails(detailProgressBar);
				for (TargetDetails target : targets) {
					if (target.getNotInRange()) {
						if (JOptionPane.showOptionDialog(progressFrame,
								...
								"Warnung", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, null,
								null) == JOptionPane.YES_OPTION) {
							goOn = true;
						} else {
							goOn = false;
				...
				progressInfo.setText("Ermittle gültige Positionen für Haltestellen...");
				progressBar.setValue(progressBar.getValue() + 1);

				progressInfo.setText("Erstelle OR-Modell ...");
				detailProgressBar.setValue(0);
				double[][] orModel = OrUtil.createOrMatrix(targets, detailProgressBar);

				// OrOptiIterator it = OrOptiIterator.getInstance(orModel);
				OrOptiIterator it = OrOptiIterator.createNewSingletonInstance(orModel);
				...
				if (resultList.size() == 0) {
					JOptionPane.showMessageDialog(View.getInstance(), "Keine Lösungen für die aktuelle Optimierung");
					optimierungRun = false;
					return;
				}
				...
				e.printStackTrace();
				JOptionPane.showMessageDialog(View.getInstance(),
						"Bei der Optimierung ist folgender Fehler aufgetreten: " + e.getMessage(), "Bus Simulation",
						JOptionPane.ERROR_MESSAGE);
				optimierungRun = false;
				return;
			}
	...
	public void updateEditor(int resultId) {
		...
		if (resultId <= resultList.size()) {
			...
			if (result != null) {
				...
				System.out.println(planquadrate.length);
				System.out.println(planquadrate[1].length);
				for (int i = 0; i < v.length; i++) {
					...
					if (v[i].getValue() == 1) {
						...
						System.out.println("Haltestelle "+i+1+" Koordinaten " +x+" / "+y);
						SimuPanel.getInstance().insertHaltestelle(planquadrate[x][y].getPixelCoordinates(),
								"Haltestelle " + (i + 1));
						}
					}
				}
	...
	public void createResultTable() {
		...
		for (int i = 0; i < resultList.size(); i++) {
			...
			if (result != null) {
				...
				date = resultDate.get(Calendar.DATE) + "." + (resultDate.get(Calendar.MONTH) + 1) + "."
						+ resultDate.get(Calendar.YEAR);
				rowData[i][1] = date;

				time = resultDate.get(Calendar.HOUR_OF_DAY) + ":" + resultDate.get(Calendar.MINUTE) + ":"
		...
		listSelectionModel
				.addListSelectionListener(new TableListSelectionListener(table, solutionInfoPanel, resultList));
