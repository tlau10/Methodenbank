...
public class SimuPanel extends JLayeredPane {
	...
	public void initialize() {
		...
		halteStellen = new ArrayList<Haltestelle>(Strassennetz.getInstance().getAlleHaltestellen());
	}
	...
	private SimuPanel() {
		this.initialize();
		this.setLayout(null);
		this.background = Toolkit.getDefaultToolkit().getImage(ImageUtil.getImageUrl("Eisenstadt.jpg"));
		...
		this.graph.setPreferredSize(
				new Dimension(this.anzahlFelder * (this.sizeOfField), this.anzahlFelder * (this.sizeOfField)));
		...
		this.graph.getModel().addGraphModelListener(new LockedHaltestellenListener());
		this.graph.getModel().addGraphModelListener(new LockedPlanquadratListener());
	...
	public void startAnimation(BusSimulationModel bsm) {
		...
		for (int i = 0; i < busse.size(); i++) {
			...
			Logger.getInstance()
					.log("Bus-Position: " + busse.get(i).getPositionX() + " " + busse.get(i).getPositionY());
	...
	public void updateView(Bus aBus) {
		for (int i = 0; i < busPanels.size(); i++) {
			if (busPanels.get(i).getBus().getId() == aBus.getId()) {
				busPanels.get(i).setBounds(aBus.getPositionX(), aBus.getPositionY(), 100, 100);
			}
		}
	}
	...
	public void insertHaltestelle(Point2D point, String name) {
		HaltestellenCell vertex = createHaltestellenCell(halteStellen.size() + 1, name, 1);
		vertex.getAttributes().applyMap(createCellAttributes(point, "Haltestelle"));
		graph.getGraphLayoutCache().insert(vertex);
		int x = OrUtil.getPlanquadratXCoordinate((int) graph.getCellBounds(vertex).getCenterX());
		int y = OrUtil.getPlanquadratYCoordinate((int) graph.getCellBounds(vertex).getCenterY());
	...
	public void removeHaltestellen() {
		Object[] haltestellen = graph.getGraphLayoutCache().getCells(false, true, false, false);
	...
	public void removeHaltestelle(Haltestelle h) {
		...
		Object o[] = graph.getGraphLayoutCache().getCells(false, true, false, false);
	...
	public void resetPanel() {
		...
		Object[] cells = simuPanel.getGraph().getGraphLayoutCache().getCells(true, true, true, true);
	...
	public void insertZiel(Point2D point) {
		...
		if (anzahlZiele ==14) {
			JOptionPane.showMessageDialog(View.getInstance(), "Sie haben nun 15 Ziele hinzugefügt. Mehr als 15 Ziele kann sich stark auf die Performance Ihres Computers auswirken.",
					"Performance Warnung", JOptionPane.WARNING_MESSAGE);
		}
		if (anzahlZiele > 14) {
			JOptionPane.showMessageDialog(View.getInstance(), "Sie haben mehr als 15 Ziele hinzugefügt. Dies kann sich stark auf die Performance Ihres Rechners auswirken. Es wird empfohlen, die Anzahl der Ziele geringer zu halten!",
					"Performance Warnung", JOptionPane.WARNING_MESSAGE);
		}
	...
	public void insertGesperrteHaltestelle(Point2D point) {
		...
		ArrayList<GesperrteHaltestelleCell> gesperrteHaltestellenListe = this.getGesperrteHaltestellenCells();
		if (!gesperrteHaltestellenListe.isEmpty()) {
			anzahlGesperrteHaltestelle = gesperrteHaltestellenListe.size();
		}
		GesperrteHaltestelleCell vertex = this.createGesperrteHaltestelleCell(anzahlGesperrteHaltestelle);

		vertex.getAttributes().applyMap(createCellAttributes(point, "gesperrteHaltestelle"));
		graph.getGraphLayoutCache().insert(vertex);
	}
	...
	public void insertStrasseAfterLoading(Point2D point, Point2D endPoint) {
		...
		edge.setName("Strasse");
		edge.setId(anzahlStrassen);
		edge.getAttributes().applyMap(this.createEdgeAttributesForLoading(point, endPoint, "Strasse"));
		graph.getGraphLayoutCache().insertEdge(edge, null, null);
	}
	 ...
	 * @param endPoint
	 */
	public void insertGesperrteStrasseAfterLoading(Point2D point, Point2D endPoint) {
		int anzahlStrassen = 0;
		List<GesperrteFelderEdge> strassenListe = this.getGesperrteFelderEdges();
		if (!strassenListe.isEmpty()) {
			anzahlStrassen = strassenListe.size();
		}
		...
		edge.setId(anzahlStrassen);
		edge.getAttributes().applyMap(this.createEdgeAttributesForLoading(point, endPoint, "GesperrteFelder"));
		graph.getGraphLayoutCache().insertEdge(edge, null, null);
	}
	...
	public void insertGesperrteFelder(Point2D point) {
		...
		List<GesperrteFelderEdge> gesperrteFelderListe = this.getGesperrteFelderEdges();
		...
		edge.getAttributes().applyMap(createEdgeAttributes(point, "GesperrteFelder"));
		graph.getGraphLayoutCache().insertEdge(edge, null, null);
	}
	...
	public Map createCellAttributes(Point2D point, String s) {
		...
		if (s.compareTo("Haltestelle") == 0) {
			Icon icon = new ImageIcon(ImageUtil.getImageUrl("haltestelle_klein.png"));
			...
			Icon icon = new ImageIcon(ImageUtil.getImageUrl("ghaltestelle_klein.png"));
			GraphConstants.setIcon(map, icon);
			// GraphConstants.setBounds( map, new Rectangle2D.Double(
			// point.getX(),
			// point.getY(), 0, 0 ) );
		...
		GraphConstants.setBounds(map,
				new Rectangle2D.Double(point.getX(), point.getY(), this.sizeOfField, this.sizeOfField));
	 ...
	 * @return
	 */
	private HaltestellenCell createHaltestellenCell(int i, String name, int kapazitaet) {
		HaltestellenCell cell = new HaltestellenCell(i, name, kapazitaet);
		DefaultPort port = new DefaultPort();
		GraphConstants.setOffset(port.getAttributes(), new Point(0, GraphConstants.PERMILLE / 2));
		Point portPoint = new Point(GraphConstants.PERMILLE / 2, GraphConstants.PERMILLE);
	...
	private Map createEdgeAttributes(Point2D point, String s) {
		...
		if (s.compareTo("Strasse") == 0 || s.compareTo("GesperrteFelder") == 0) {
			...
			labelPosition.setLocation(graph.getWidth() * 100, graph.getHeight() * 100);
	 ...
	 */
	@SuppressWarnings("rawtypes")
	private Map createEdgeAttributesForLoading(Point2D point, Point2D endPoint, String s) {
		Map map = new Hashtable();
	 ...
	 */
	@SuppressWarnings("rawtypes")
	private Map createEdgeAttributesForLoadingALine(Point2D point, String s, Linie linie) {
		Map map = new Hashtable();
	...
	public void connectHsWithLine(Linie buslinie) {
		...
		if (teilStrecken.size() > 1) {
			...
			for (Teilstrecke teilStrecke : teilStrecken) {
				...
				if (myMarquee.getSourcePortAt(s) != null) {
					...
					for (int i = 0; i < tArray.size(); i++) {
						if (tArray.get(i).getAttributes() == source.getAttributes()) {
							exists = true;
						}
					}
					if (exists == false) {
				...
				if (myMarquee.getTargetPortAt(e) != null) {
					...
					boolean exists = false;
					for (int i = 0; i < tArray.size(); i++) {
						if (tArray.get(i).getAttributes() == target.getAttributes()) {
							exists = true;
						}
					}
					if (exists == false) {
			...
			for (int i = 0; i < tArray.size() - 1; i++) {
				...
				this.connectSavedHaltestellen(source, target, buslinie, anzahlTeilstrecken);
				anzahlTeilstrecken++;
			}
	...
	public void connectSavedHaltestellen(Port source, Port target, Linie bus, int anzahl) {
		if (Strassennetz.getInstance().getArrayBuslinie().size() != 0) {
			...
			if (graph.getModel().acceptsSource(edge, source) && graph.getModel().acceptsTarget(edge, target)) {
				...
				edge.getAttributes().applyMap(createEdgeAttributesForLoadingALine(null, "Linie", bus));
			...
			JOptionPane.showMessageDialog(this, "Bitte wählen Sie zuerst eine Linie aus", "Keine Linie ausgewählt",
	...
	public void connect(Port source, Port target) {
		...
		if (Strassennetz.getInstance().getArrayBuslinie().size() != 0) {
			Set s = DefaultGraphModel.getEdges(graph.getModel(),
			...
			if (graph.getModel().acceptsSource(edge, source) && graph.getModel().acceptsTarget(edge, target)) {
				...
				Object[] insert = new Object[] { edge };
				ConnectionSet cs = new ConnectionSet();
				edge.getAttributes().applyMap(createEdgeAttributes(null, "Linie"));
				edgeAttribute = edge.getAttributes();
				m_mapAttribute.put(edge, edgeAttribute);
				cs.connect(edge, source, target);
				graph.getGraphLayoutCache().insertEdge(edge, source, target);
	...
	public void selectAllLinien() {
		...
		for (int i = 0; i < obj.length; i++) {
			...
			if (edge.toString().compareTo("Strasse") == 0 || edge.toString().compareTo("GesperrteFelder") == 0) {
	...
	public void selectHaltestelle(String s) {
		Object[] obj = graph.getGraphLayoutCache().getCells(false, true, false, false);
	...
	public void deleteSelectedLinienEdge() {
		SimuControlPanel simuControl = SimuControlPanel.getInstance();
		...
		for (int i = 0; i < obj.length; i++) {
			DefaultEdge edge = (DefaultEdge) obj[i];
			if (edge.toString().equals(bus.getId()+"_"+(simuControl.getSelectedTeilstrecke().getId()-1))) {
				selObj.add(edge);
			}
		}
		Object[] selectObj = new Object[selObj.size()];
		...
		bus.deleteTeilstrecke(simuControl.getSelectedTeilstrecke().getId());
	...
	private List<StrassenEdge> getStrassenEdges() {
		Object[] o = graph.getGraphLayoutCache().getCells(false, false, false, true);
	...
	public void setColorSelectedLinie(Color aColor) {
		...
		for (int i = 0; i < selectedLinien.length; i++) {
			GraphConstants.setLineColor(selectedLinien[i].getAttributes(), aColor);
		}
		graph.getGraphLayoutCache().reload();
		graph.repaint();
