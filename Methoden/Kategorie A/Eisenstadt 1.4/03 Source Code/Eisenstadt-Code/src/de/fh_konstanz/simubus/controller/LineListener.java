...
/**
 * Die Klasse <code>LineListener</code> reagiert auf Ereignisse, wenn dem Modell
 * eine Linie hinzugefuegt wird.
 * 
...
public class LineListener implements GraphModelListener {
	...
	public void graphChanged(GraphModelEvent arg0) {
		...
		// get the values of StrassenEdge and GesperrteFelderEdge for
		// XMLFileManager
		...
		if (changed != null && changed.length > 0) {
			for (int i = 0; i < changed.length; i++) {
				if (changed[i] instanceof StrassenEdge) {
					...
					GesperrteHaltestelle h = new GesperrteHaltestelle(theGesperrteHaltestellenEdge.getKoordinaten(),
							theGesperrteHaltestellenEdge.getId());
						...
						if (GraphConstants.getPoints(e.getAttributes()) != null) {
							...
							for (int k = 0; k < points.length; k++) {
								if (k + 1 < points.length) {
									...
									HaltestellenCell h1 = (HaltestellenCell) graph.getModel()
											.getParent(graph.getModel().getSource(e));
									HaltestellenCell h2 = (HaltestellenCell) graph.getModel()
											.getParent(graph.getModel().getTarget(e));
