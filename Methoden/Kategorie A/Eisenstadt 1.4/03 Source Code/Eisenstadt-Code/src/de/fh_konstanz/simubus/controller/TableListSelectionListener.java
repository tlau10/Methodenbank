...
public class TableListSelectionListener implements ListSelectionListener {
	 ...
	 * @param results
	 */
	public TableListSelectionListener(JTable table, JPanel resultInfoPanel, List<Result> results) {
		this.table = table;
		...
		if (!e.getValueIsAdjusting()) {
			...
			for (int i = 0; i < table.getColumnCount(); i++) {
				if (table.getColumnName(i).equals("Lösung") == true) {
					resultId = Integer.parseInt(table.getValueAt(selectedRow, i).toString()) - 1;
					break;
				}
			}
			...
			if (results.size() > 0) {
				...
				for (int i = 0; i < variables.length; i++) {
					if (variables[i].getValue() == 1) {
						int j = i;
						int y = OrUtil.getYCoordinate(variables[j]);
						if (y ==64) {
							j--;
							y = OrUtil.getYCoordinate(variables[j]);
						}
						int x = OrUtil.getXCoordinate(variables[j], y);

						x = OrUtil.getPixelXCoordinate(x);
						y = OrUtil.getPixelYCoordinate(y);

						text = text.concat("[x: " + String.valueOf(x) + " y: " + String.valueOf(y) + "]");
				...
				for (int i = 0; i < details.length; i++) {
					...
					text = text.concat("[x: " + (details[i].getPlanquadrat().getPixelX() + 6) + " y: "
							+ (details[i].getPlanquadrat().getPixelY() + 6) + "]: "
							+ OrUtil.getTime(solution, details[i]) + " Minuten (");
	 ...
	 * @param targetDetails
	 */
	public void showResultsInfo(ArrayList<String> basicSolution, ArrayList<String> targetDetails) {
		...
		resultInfoTable.getTable().setSelectionMode(ListSelectionModel.SINGLE_INTERVAL_SELECTION);
		ListSelectionModel listSelectionModel = resultInfoTable.getTable().getSelectionModel();
		listSelectionModel.addListSelectionListener(new ResultInfoTableSelectionListener());

		resultInfoPanel.revalidate();
		resultInfoTable.getTable().addMouseListener(new SelectTable(resultInfoTable));
	 ...
	 * 
	 */
	public class ResultInfoTableSelectionListener implements ListSelectionListener {

		public void valueChanged(ListSelectionEvent arg0) {
			resultInfoTable.getTable().addMouseListener(new OptiSelectListener(resultInfoTable.getTable()));
