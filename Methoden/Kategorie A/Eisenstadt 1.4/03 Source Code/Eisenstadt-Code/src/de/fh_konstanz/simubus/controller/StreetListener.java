...
/**
 * Die Klasse <code>StreetListener</code> reagiert darauf, falls eine Strasse in
 * das Modell eingefuegt, verschoben oder geloescht wird.
 * 
				...
				if (deleted[i] instanceof StrassenEdge) {
					for (int j = 0; j < planquadrate.length; j++) {
						for (int k = 0; k < planquadrate[j].length; k++) {
							...
							if (planquadrate[j][k].isStreet && (planquadrate[j][k].getID() == edge.getId())) {
					...
					for (int j = 0; j < planquadrate.length; j++) {
						for (int k = 0; k < planquadrate[j].length; k++) {
							...
							if (edgeView.intersects(graph, r) == true) {
								...
								if (planquadrate[j][k].isStreet == true
