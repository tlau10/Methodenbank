...
public class Table {
	...
	public JScrollPane getScrollPane(String tableName) {
		...
		table = new JXTable(tableModell) {
			...
			public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
				...
				for (int i = 0; i < selectedTableItems.size(); i++) {
					TableColor tableColor = (TableColor) selectedTableItems.get(new Integer(i));

					if (row == tableColor.getRow() && column == tableColor.getColumn())
		...
		table.setSize(new Dimension(this.getTableWidth(), table.getRowCount() * table.getRowHeight()));
		...
		if (table.getName().equals("Ergebnisse") == true) {
			...
			int columnIndex = tableModell.getColumnIndex(String.valueOf("Lösung"));
			...
			columnIndex = tableModell.getColumnIndex(String.valueOf("Durchschnittszeit"));
