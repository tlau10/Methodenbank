...
/**
 * Die Klasse <code>OptiSelectListener</code> ist ein Listener, welche die
 * optimierte Haltestelle sowie auch die Ziele auf die Karte
 * markiert(hervorhebt).
 * 
 * 
...
public class OptiSelectListener extends MouseAdapter {
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (table.getColumnName(selectedColumn).equals("Haltestellen") == true) {
			coordinates = table.getValueAt(selectedRow, selectedColumn).toString();
	...
	public void selectZiele() {
		...
		for (ZieleCell ziel : ziele) {
			...
			if (ziel.getKoordinaten().x == x && ziel.getKoordinaten().y == y) {
				...
				Object cell = simuGraph.getFirstCellForLocation(pixelXCoordinate, pixelYCoordinate);

				if (cell instanceof ZieleCell) {
					ZieleCell zieleCell = ((ZieleCell) cell);
					lastSelectZiel = zieleCell;
					zieleCell.getAttributes().applyMap(simuPanelInstance
							.createCellAttributes(new Point(pixelXCoordinate, pixelYCoordinate), "optimierteZiele"));
	...
	public void selectHaltestelle() {
		...
		Collection<Haltestelle> haltestellen = simuPanelInstance.getAllHaltestellen();

		for (Haltestelle haltestelle : haltestellen) {

			if (haltestelle.getKoordinaten().x == x && haltestelle.getKoordinaten().y == y) {
				...
				if (cell instanceof HaltestellenCell) {
					...
					halteStellenCell.getAttributes().applyMap(simuPanelInstance.createCellAttributes(
							new Point(pixelXCoordinate, pixelYCoordinate), "optimierteHaltestelle"));
	...
	public void deselectAll() {
		...
		if (lastSelectHaltestelle != null) {
			...
			if (cell instanceof HaltestellenCell) {
				HaltestellenCell halteStellenCell = ((HaltestellenCell) cell);
				halteStellenCell.getAttributes().applyMap(simuPanelInstance
						.createCellAttributes(new Point(pixelXCoordinate, pixelYCoordinate), "deselect"));
				simuGraph.getGraphLayoutCache().insert(halteStellenCell);
		...
		if (lastSelectZiel != null) {
			...
			pixelXCoordinate = OrUtil.getPixelXCoordinate((int) lastSelectZiel.getKoordinaten().getX());

			pixelYCoordinate = OrUtil.getPixelXCoordinate((int) lastSelectZiel.getKoordinaten().getY());
