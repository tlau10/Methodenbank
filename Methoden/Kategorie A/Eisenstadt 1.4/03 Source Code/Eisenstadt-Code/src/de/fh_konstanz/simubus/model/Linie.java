...
public class Linie implements Serializable {
	...
	public void setPunkte(int i, Point2D start, Point2D ende) {
		Teilstrecke teilstrecke = new Teilstrecke(i, "Teilstrecke " + String.valueOf(this.getTeilstrecken().size() + 1),
				start, ende, 50, 10);
	...
	public void deleteTeilstrecke(int id) {
		Teilstrecke toDelete = null;
		for (Teilstrecke teilstrecke: teilstrecken) {
			if (teilstrecke.getId() == id) {
				toDelete = teilstrecke;
				break;
			}
		}
		if (toDelete != null) {
			teilstrecken.remove(toDelete);
			haltestellen.clear();
		}
	}
}
