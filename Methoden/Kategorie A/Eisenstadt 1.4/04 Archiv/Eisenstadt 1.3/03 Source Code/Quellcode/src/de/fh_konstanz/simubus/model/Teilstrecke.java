...
public class Teilstrecke implements Serializable {
	 ...
	 */
	public Teilstrecke(int id, String name, Point2D start, Point2D ende,
			int geschwindigkeit, int breite) {
		this.id = id;
		this.name = name;
		this.start = start;
		this.ende = ende;
		...
		this.breite = breite;
		this.laenge = Math.sqrt(quadrat(ende.getY() - start.getY())
				+ quadrat(ende.getX() - start.getX()));
	}
