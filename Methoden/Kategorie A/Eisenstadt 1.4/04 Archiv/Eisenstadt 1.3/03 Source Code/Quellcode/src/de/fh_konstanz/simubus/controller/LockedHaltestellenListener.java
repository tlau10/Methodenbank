...
/**
 * Die Klasse <code>LockedHaltestellenListener</code> reagiert darauf, falls eine
 * gesperrte Haltestelle in das Modell eingefuegt, verschoben oder geloescht wird.
 * 
	            ...
	            if ( changed[ i ].toString() != null )
	            {
	               if ( changed[ i ] instanceof GesperrteHaltestelleCell )
	               {
	                  ...
	                  for ( int j = 0; j < planquadrate.length; j++ )
	                  {
	                     ...
	                     {
	                        Rectangle2D rec = GraphConstants.getBounds( ( (GesperrteHaltestelleCell) changed[ i ] )
	                              .getAttributes() );

	                        Rectangle2D.Double r = planquadrate[ j ][ k ].getBounds();

	                        if ( planquadrate[ j ][ k ].isLockedHaltestelle == true
		                              && planquadrate[ j ][ k ].getID() == ( (GesperrteHaltestelleCell) changed[ i ] ).getId() )
	                        {
	                          GesperrteHaltestelle gesperrteHaltestelle = new GesperrteHaltestelle( planquadrate[ j ][ k ].getPlanquadratCoordinates(), z
		                                 .getId() );
	                           planquadrate[ j ][ k ].setEmpty();
	                           Strassennetz.getInstance().removeGesperrteHaltestelle(
	                        		   gesperrteHaltestelle );
