...
/**
 * Die Klasse <code>HaltestelleListener</code> reagiert darauf, wenn eine Haltestelle
 * dem Modell hinzugefuegt wird, eine Haltestelle verschoben wird oder eine
 * Haltestelle geloescht wird.
 * 
            ...
            if ( changed[ i ].toString() != null )
            {
               if ( changed[ i ] instanceof HaltestellenCell )
               {
                  ...
                  for ( int j = 0; j < planquadrate.length; j++ )
                  {
                     for ( int k = 0; k < planquadrate[ j ].length; k++ )
                     {
                        Rectangle2D rec = GraphConstants.getBounds( ( (HaltestellenCell) changed[ i ] )
                              .getAttributes() );
                        ...
                        {
                           Haltestelle h = new Haltestelle( haltestelleCell.getId(), planquadrate[ j ][ k ]
                                 .getPlanquadratX(), planquadrate[ j ][ k ].getPlanquadratY(),
                           ...
                           if ( Strassennetz.getInstance().getStrassenListePlanquadrat().contains(
                                 planquadrate[ j ][ k ] ) == false )
                           {
                           ...
                           haltestelleCell.setKoordinaten( planquadrate[ j ][ k ].getPlanquadratX(),
                              ...
                              && planquadrate[ j ][ k ].getID() == ( (HaltestellenCell) changed[ i ] )
                                    .getId() )
                        {
                        ...
                        ArrayList<Haltestelle> halteStellen = new ArrayList<Haltestelle>( Strassennetz
                              .getInstance().getAlleHaltestellen() );
