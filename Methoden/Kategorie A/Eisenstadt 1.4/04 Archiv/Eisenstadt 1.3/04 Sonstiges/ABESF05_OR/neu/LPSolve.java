...
public class LPSolve implements Solver {
  ...
  private boolean detailliertesErgebnis;
  ...
  public void setProperties(int[][] abstandsMatrix_, ArrayList gewichtungsMatrix_,  matrixPlanQuadratController mPQC_, boolean detailliertesErgebnis_)
  {
  	detailliertesErgebnis = detailliertesErgebnis_;
  	mPQC = mPQC_;
    ...
    {
    	for(int y=0; y < myHSVector.size(); y++)
    	{
    		optimaleHalteStellen[x][y] = "nichtBelegt";
    	}
    }
  }
  ...
  public String parseSolution(int durchlauf)
  {
	String solution = new String();
  	...
  	for(int b=0; b < myHSVector.size(); b++)
  	{
  		...
  		{ 
  			gesamtAufkommen=0;
  			optimaleHalteStellen[durchlauf][b] = (String)myHSVector.get(b);
  			solution += "Haltestelle in Feld " + myHSVector.get(b) + " wird gebaut.\n";
  				...
  				{	
  					...
  					{
  						if(this.detailliertesErgebnis) solution += "     "+temp+" Personen -> Feld " + ((c+1)-(b*laenge*breite))+"\n";
