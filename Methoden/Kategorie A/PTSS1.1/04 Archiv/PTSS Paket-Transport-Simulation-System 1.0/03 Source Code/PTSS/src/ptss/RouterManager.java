...
//******************************************************************************
 public  synchronized void doAlleRouten(){
    Knoten tmpVonKnoten, tmpNachKnoten;

    // test mje
    int routenAnz = KnotenListe.getSize()*(KnotenListe.getSize()-1);

    mView.StartProgress(routenAnz);
    //   progressWindow = new Progress(routenAnz,this);
    ...
    for (Iterator iterVon = KnotenListe.iterator(); iterVon.hasNext(); ) {
      tmpVonKnoten = (Knoten) iterVon.next();

      for (Iterator iterNach = KnotenListe.iterator(); iterNach.hasNext(); ) {
        tmpNachKnoten = (Knoten) iterNach.next();

        if((tmpVonKnoten.getId() == tmpNachKnoten.getId()))
          continue;

        if(!routenListe.existsRoute( tmpVonKnoten.getId(), tmpNachKnoten.getId() )){
          KnotenListe.setStartKnoten(tmpVonKnoten.getId());
          KnotenListe.setZielKnoten(tmpNachKnoten.getId());
          //Solver setzen
          setSolverLPSolve();
          route = new Route();
          //Solver anstossen
          ...
          KnotenListe.clearStart();
          KnotenListe.clearZiel();
        }
        mView.setCalculatedRouten(anzahlRouten++); // für progress Window
 //       System.out.println(anzahlRouten++);
      }
    }
 //   routenListe.display();
 //   progressWindow.dispose(); // added mje
    mView.StopProgressPanel();
