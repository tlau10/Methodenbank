...
public class ModellierView extends JPanel{
  ...
  public void saveModell(){
    ...
    if (fileToSave != null){
      ...
      JOptionPane.showMessageDialog(null, ""+fileToSave );
...
//      mManager.getKantenListe().clearPath();
     ...
     try{
    	 ObjectWriter out = new ObjectWriter( new FileWriter(fileToSave) );
/*
...
*/
  ...
  public void openModell(){
	    ...
	    if (fileToOpen != null){
	      ...
	      RouterManager rManager = new RouterManager();
	      try{
	    	ObjectReader in = new ObjectReader( new FileReader(fileToOpen));
	        ...
	        System.out.println("Lesefehler "+ e2.getMessage());
	      }
...
public ModellierManager getManager(){
  ...
  void route_btn_all_actionPerformed(ActionEvent e) {
	...
	*/
	    ...
	    text += "Gesamt Anzahl Strecken: " + rm.getRoutenAlleListe().size() + "\n\n\n";
	    
	    
	    // Fester mit Ausgabe der RoutenInfo
	    Route tmpRoute = new Route();
	    
		for (Iterator iterNach = rm.getRoutenAlleListe().iterator(); iterNach.hasNext(); )
		{
			tmpRoute = (Route) iterNach.next();
		    ...
		    text += "Start-Knoten: " + mManager.getKnotenListe().getName(tmpRoute.nextKnoten())+"\n";
		    while (tmpRoute.hasNextKnoten()){
		      text += "  via Knoten: " + mManager.getKnotenListe().getName(tmpRoute.nextKnoten())+"\n";
		    }
		    text += " Ziel-Knoten: " + mManager.getKnotenListe().getName(tmpRoute.getZiel())+"\n";
		    text += "Gesamtkosten: " + rm.getGesamtkostenOfRoute(tmpRoute);
