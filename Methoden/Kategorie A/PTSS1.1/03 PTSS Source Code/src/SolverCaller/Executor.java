...
public class Executor implements IErrorMessage
{
  ...
  */
  public    synchronized  int starteProcess(String execPath, String errFile, String outFile)
  {
...
//        System.out.println(execPath);
        Process proc = rt.exec("temp\\start_lpsolve.bat");
   ...
   //   errorGobbler.join(100);        // max. 4 Sek warten, dann aber weiter..

      outputGobbler.start();
   //   while(outputGobbler.isAlive());  // Warten bis Thread-Ende(gefährlich!)
   //   outputGobbler.join(100);    // max. 3 Sek. warten, dann ist gut..
