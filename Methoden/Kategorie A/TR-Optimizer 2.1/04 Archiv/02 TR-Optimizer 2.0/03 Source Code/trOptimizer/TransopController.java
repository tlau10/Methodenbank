...
public class TransopController {
  ...
  public boolean isTransporterInPos( double x, double y ) {
    ...
    for( int i = 0; i < transporter.size(); i++ ) {
      ...
      if( x >= t.getX() && x <= ( t.getX() + t.getBreite() ) && y >= ( t.getY() - 15 ) &&
...
/*
      ...
      ergebnis += " Transporter "+( ( Ergebnis )erg.get( i ) ).getTransporter();
      ergebnis += " Ladung "+( ( Ergebnis )erg.get( i ) ).getLadung();
      ergebnis += " Quelle "+( ( Ergebnis )erg.get( i ) ).getQuelle();
      ergebnis += " Ziel "+( ( Ergebnis )erg.get( i ) ).getZiel();
      ergebnis += " Kosten "+( ( Ergebnis )erg.get( i ) ).getKosten();
*/
    }
    System.out.println( "\nErgebnis-Ende" );
