...
public class Solve {
	...
	// diese Funktion ueberprueft ob die Liefermenge mit der empfangenen Menge
	// uebereinstimmt
	// wenn dies nicht so ist wird ein virtueller Empfaenger/ Produzent
	// eingefuehrt
	private void korrigiereDaten() {
		...
		for (int i = 0; i < empfaengerliste.size(); i++) {
			empfMenge += ((Empfaenger) empfaengerliste.get(i))
					.getBenoetigteMenge();
		}
		if (prodMenge > empfMenge) {
			...
			for (int i = 0; i < produzentenliste.size(); i++) {
				...
				virtStrecke.setQuelle(((Produzent) produzentenliste.get(i))
						.getId());
		...
		if (prodMenge < empfMenge) {
			...
			for (int i = 0; i < empfaengerliste.size(); i++) {
				Strecke virtStrecke = new Strecke(-50, -50, -50, -50, 0);
				virtStrecke.setEntfernung(0);
				virtStrecke.setQuelle(virtProd.getId()); // Quelle immer Prod.,
															// Ziel immer Epmf.
				virtStrecke.setZiel(((Empfaenger) empfaengerliste.get(i))
						.getId());
				streckenliste.add(virtStrecke);
			}
		}
	...
	// Im Konstruktor werden gleich alle benoetigten Funktionen aufgerufen damit
	// sofort eine Loesung vorliegt welche durch die "get-Methode" abgeholt
	// werden kann
	public Solve(int solvertyp, Vector empfaengerliste,
			Vector produzentenliste, Vector streckenliste,
			Vector transporterliste) {
		this.empfaengerliste = empfaengerliste;
		this.produzentenliste = produzentenliste;
		this.streckenliste = streckenliste;
		this.transporterliste = transporterliste;
		this.solvertyp = GlobaleVariable.solvertyp;
		...
		if (GlobaleVariable.solvertyp == 2) {
			this.createFileWeid();
			if (calculateWeidenauer() == true)
				parseDataWeid();
		}
		if (GlobaleVariable.solvertyp == 3) {
			this.createMopsFile();
	...
	// ///////////////////////////////////////////////////////////////////////////
	// In dieser Funktion wird das komplette LP-Modell fuer Mops erstellt!
	private void createMopsFile() {
		...
		for (int i = 0; i < transporterliste.size(); i++) {
			for (int j = 0; j < streckenliste.size(); j++) {
				zf += " + "
						+ (((Strecke) streckenliste.get(j)).getEntfernung() * ((Transporter) transporterliste
								.get(i)).getKosten_pro_km()) + " x "
						+ koeffizient;
				koeffizient++;
			}
		}
		...
		// Restriktionen fuer die Produzenten
		for (int i = 0; i < produzentenliste.size(); i++) {
			rest = "R " + restNr + " :  ";
			for (int j = 0; j < streckenliste.size(); j++) {
				if ((((Strecke) streckenliste.get(j)).getQuelle()) == (((Produzent) produzentenliste
						.get(i)).getId())) {
					if (koeffizient == 1) {
			...
			rest += " = "
					+ ((Produzent) produzentenliste.get(i)).getLieferMenge()
					+ " ;";
			dataSet += rest + "\n";
			koeffizient = 1;
			restNr++;
		...
		// Restriktionen fuer die Empfaenger
		for (int i = 0; i < empfaengerliste.size(); i++) {
			rest = "R " + restNr + " :  ";
			for (int j = 0; j < streckenliste.size(); j++) {
				if (((Strecke) streckenliste.get(j)).getZiel() == ((Empfaenger) empfaengerliste
						.get(i)).getId()) {
					if (koeffizient == 1) {
						rest += " 1 x " + koeffizient;
					} else {
			...
			rest += " = "
					+ ((Empfaenger) empfaengerliste.get(i))
							.getBenoetigteMenge() + " ;";
			dataSet += rest + "\n";
			koeffizient = 1;
			restNr++;
		...
		for (int i = 0; i < streckenliste.size(); i++) {
			...
			for (int k = 0; k < transporterliste.size(); k++) {
				for (int l = 0; l < streckenliste.size(); l++) {
					if (l == firstPosition) {
						rest += " - "
								+ ((Transporter) transporterliste.get(k))
										.getKapazitaet() + " x " + koeffizient;
					}
					koeffizient++;
				}
			}
		...
		try {
			java.io.FileWriter fileWriter6 = new java.io.FileWriter(
					IniPaths.getTempPath() + "\\MopsDatei.mps");
			java.io.PrintWriter batchWriter6 = new java.io.PrintWriter(
					fileWriter6);
	...
	private String createMopsFormat() {
		...
		// Daten einlesen
		try {
			java.io.FileReader mopsOutReader = new java.io.FileReader(
					IniPaths.getTempPath() + "\\MopsDatei.mps");
			java.io.BufferedReader mopsreader = new java.io.BufferedReader(
					mopsOutReader);
			java.io.FileWriter fileWriter3 = new java.io.FileWriter(
					IniPaths.getTempPath() + "\\Mops.mps");
			java.io.PrintWriter batchWriter3 = new java.io.PrintWriter(
					fileWriter3);
			...
			for (int l = 1; l < i; l++) {
				...
				while (!(columnsmatrix[p][2].equals("q"))) {
					if (columnsmatrix[p][2].equals(s)) {
						...
						if (tmp2 < 10) {
							columnsFunktion = columnsFunktion.concat("    x"
									+ columnsmatrix[p][2] + "        "
									+ columnsmatrix[p][3] + "        "
									+ columnsmatrix[p][4] + "\n");
						}
			...
			for (int l = 1; l < 100; l++) {
				if (columnsmatrix[l][1].equals("MYRHS")) {
					rhsFunktion = rhsFunktion.concat("    MYRHS     "
							+ columnsmatrix[l][3] + "        "
							+ columnsmatrix[l][4] + "\n");
				}
			}
	...
	// diese Funktion berechnet die Loesung mit Mops
	...
	// IniPaths.getTempPath() + "\\mopsbatch.cmd");
	// java.io.PrintWriter batchWriter55 = new
	// java.io.PrintWriter(fileWritermopsbatch);
	// batchWriter55.println("@echo off");
	// batchWriter55.println("copy " + "\"" +IniPaths.getMOPSPath() +"\" " +
	// IniPaths.getTempPath());
	// batchWriter55.println("C:");
	// batchWriter55.println("chdir " + IniPaths.getTempPath());
	// batchWriter55.println("start /wait " +
	// IniPaths.getTempPath()+"\\MOPS_PC.exe " + IniPaths.getTempPath() +
	// "\\Mops.mps MIN");
	// batchWriter55.flush();
	// batchWriter55.close();
	// try
	...
	// try
	// {
	// Process mopsproc = Runtime.getRuntime().exec( IniPaths.getTempPath() +
	// "\\mopsbatch.cmd");
	// mopsproc.waitFor();
	// mopsproc.destroy();
	// return true;
	...
	private boolean calculateMops(String MopsData) {
		...
		try {
			java.io.FileWriter fileWriterMOPSProfile = new java.io.FileWriter(
					IniPaths.getTempPath() + "\\xmops.pro");
			...
			printWriter88.println("xfnmps = '" + IniPaths.getTempPath()
					+ "\\mops.mps'");
			printWriter88.flush();
			printWriter88.close();
		} catch (IOException e) {
		...
		try {
			...
			batchWriter55.println("C:");
			batchWriter55.println("chdir " + IniPaths.getTempPath());
			batchWriter55.println("start /wait " + IniPaths.getTempPath()
					+ "\\MOPS.exe " + "> " + IniPaths.getTempPath()
					+ "\\MOPS.LOG");
			batchWriter55.flush();
			batchWriter55.close();
			try {
	...
	// ////////////////////////////////////////////////////////////////////////////
	// Einlesen der MOPS-Loesung damit sie weiterverarbeitet werden kann
	private boolean parseMopsData() {
		...
		// Daten einlesen
		try {
			java.io.FileReader MopsReader = new java.io.FileReader(
					IniPaths.getTempPath() + "\\mops.lps"); // "C:\\temp\\mops.out");
			java.io.BufferedReader mreader = new java.io.BufferedReader(
					MopsReader);
	...
	// ////////////////////////////////////////////////////////////////////////
	// In dieser Funktion wird das komplette LP-Modell fuer LP-Solve erstellt!
	...
	// ////////////////////////////////////////////////////////////////////////
	// In dieser Funktion wird das komplette LP-Modell fuer Weidenauer erstellt!
	private void createFileWeid() {
		try {
			java.io.FileWriter fileWriter2 = new java.io.FileWriter(
					IniPaths.getTempPath() + "\\LP.LP");
			java.io.PrintWriter batchWriter2 = new java.io.PrintWriter(
					fileWriter2);
			...
			for (int i = 0; i < produzentenliste.size(); i++) {
				...
				rest += "="
						+ ((Produzent) produzentenliste.get(i))
								.getLieferMenge() + ";";
			...
			for (int i = 1; i < anzVariablen; i++) {
				...
				// nach BOUND muessen 5 blanks sthen
				// vor 1e6 muessen 15 blanks stehen
				// GRUND: mps-Konvention
				batchWriter2.println(" UI BOUND     x" + i
						+ "               1e6");
			}
	...
	// ////////////////////////////////////////////////////////////////////////////
	// diese Funktion berechnet die Loesung mit LP-Solver
	private boolean calculateLPsolve(String dataSet) {
		...
		String proc_input_string = "";
		try {
			Process proc = Runtime.getRuntime().exec(IniPaths.getLpSolvePath());
			// get the streams
	...
	// ////////////////////////////////////////////////////////////////////////////
	// diese Funktion berechnet die Loesung mit Weidenau Optimizer
	private boolean calculateWeidenauer() {
		try {
			java.io.FileWriter fileWriter = new java.io.FileWriter(
					IniPaths.getTempPath() + "\\weid_par.cmd");
			java.io.PrintWriter batchWriter = new java.io.PrintWriter(
					fileWriter);
			temppath = IniPaths.getTempPath();
			String weidpath = IniPaths.getWeidenauerPath();
			batchWriter.println("LOADLP " + IniPaths.getTempPath() + "\\LP.LP");
			...
			java.io.FileWriter fileWriter2 = new java.io.FileWriter(
					IniPaths.getTempPath() + "\\weid_solve.cmd");
			java.io.PrintWriter batchWriter2 = new java.io.PrintWriter(
					fileWriter2);
			batchWriter2.println("@echo off");
			batchWriter2.println("copy " + "\"" + IniPaths.getWeidenauerPath()
					+ "\" " + IniPaths.getTempPath());
			batchWriter2.println("C:");
			batchWriter2.println("chdir " + IniPaths.getTempPath());
			batchWriter2.println("start /wait " + IniPaths.getTempPath()
					+ "\\LP.exe " + IniPaths.getTempPath() + "\\weid_par.cmd");
			batchWriter2.flush();
			batchWriter2.close();
			/************************************************************/
			try {
	...
	// //////////////////////////////////////////////////////////////////////////////
	// Einlesen der Weidenauer-Loesung damit sie weiterverarbeitet werden kann
	private boolean parseDataWeid() {
		...
		// Daten einlesen
		try {
			java.io.FileReader solveOutReader = new java.io.FileReader(
					IniPaths.getTempPath() + "\\Lp.int");
			java.io.BufferedReader reader = new java.io.BufferedReader(
					solveOutReader);
			// L�sungs-Datei in String lesen
			...
			// Alle Variablen-Loesungen suchen und in Vector schreiben
			for (int i = 0; i <= Anzahl_Zeilen; i++) {
	...
	// ///////////////////////////////////////////////////////////////////////////
	// Einlesen der LP-Solve-Loesung damit sie weiterverarbeitet werden kann
	private boolean parseData() {
		String token = "";
		int savekey = 0;
		int valuekey = 0; // means 'Value of objective function' comes and this
							// means a solution is comming :-)
		int zaehler = 0;
		double tmp = 0;
	...
	// Erstellen des Ergebnisses welches durch die get-Methode geholt werden
	// kann
	private void createErgebnis() {
		...
		for (int i = 1; i <= streckenliste.size(); i++) {
			if (!myStringVector.get(i).equals("0")) {
				...
				tmpErgebnis.setQuelle(((Strecke) streckenliste.get(i - 1))
						.getQuelle());
				tmpErgebnis.setZiel(((Strecke) streckenliste.get(i - 1))
						.getZiel());
				for (int j = 0; j < transporterliste.size(); j++) {
					if (!myStringVector.get(
							i + (streckenliste.size() * (j + 1))).equals("0")) {
						anzFahrten = Integer.parseInt(myStringVector.get(
								i + (streckenliste.size() * (j + 1)))
								.toString());
						tmpErgebnis
								.setTransporter(((Transporter) transporterliste
										.get(j)).getName());
						kosten = (((Transporter) transporterliste.get(j))
								.getKosten_pro_km()
								* ((Strecke) streckenliste.get(i - 1))
										.getEntfernung() * anzFahrten);
