...
public class Chart {
	...
	private JTable chartDataTab; // contains raw data for chart - in at
									// construction
	...
	public Chart(JTable tab, ChartColourScheme cols, int numBs, int numLs,
			boolean ar) { // this constructor is passed a JTable and calculates
							// values based on size
							// min/max values for chart are calculated and all
							// elements are plotted on
							// the primary x-axis.
		this(tab, cols, numBs, numLs, ar, 0, 0, false, false);
	}

	public Chart(JTable tab, ChartColourScheme cols, int numBs, int numLs,
			boolean ar, int max, int min, boolean manMax, boolean label) { // this
																			// constructor
																			// is
																			// used
																			// when
		...
		showXAxisLabels = label; // these should be set by the class
									// instantiating
		showYAxisLabels = label; // these should be set by the class
									// instantiating
		...
		numOfSeries = numLs + numBs; // here calculating the total number of
										// bars,lines
		if (areaToChart)
			numOfSeries++; // and areas that are to be drawn

		rowTitles = new String[numOfSeries]; // to store the names of the series
												// to be charted
		rowTitleIndex = 0;

		numOfColumns = chartDataTab.getColumnCount() - 1; // the table contains
															// an extra column
		// System.out.println("Num of Cols = " + numOfColumns); // with the
		// series titles
		...
		for (int cl = 0; cl < numOfColumns; cl++) // set the column headings
													// (row 1 of tab)
		{
		...
		setUpBarArray(); // current implementation neccessitates at least 1 bar
							// set
		if (linesToChart)
			setUpLineArray();
		if (areaToChart)
	...
	public void setSeriesLabels(boolean vis)// this property can be set without
											// needing to reset
	...
	// Zusatz für Aufruf der graphischen Buchungsanzeige
	public ChartPanel getPanel(int buchung, int dauer, String labelAlt,
			String labelBuch) {
		chartPanel = new ChartPanel(chartColors, this, buchung, dauer,
				labelAlt, labelBuch);
		return chartPanel;
	}
	...
	public double getLineRef(int a, int b) // for calculations by ChartPanel
											// object
	{
		return lineRefs[a][b];
	}
	...
	public double returnPrimaryRange() {
		if (chartPMax > chartPMin)
			return (chartPMax - chartPMin);
		else
			return (chartPMin - chartPMax);
	}
	...
	public void setUpAreaArray() {
		areaRefs = new int[numOfColumns + 2]; // an extra point for starting
												// point & returning
		// to starting point...

		for (int t = 0; t < numOfColumns + 1; t++) {
			objBuf = chartDataTab.getValueAt((numOfBarSeries + numOfLines + 1),
					(t));
			strBuf = objBuf.toString();
		...
		areaRefs[numOfColumns + 1] = areaRefs[numOfColumns]; // to return to
																// edge
		...
		for (int lc = numOfBarSeries; lc < (numOfBarSeries + numOfLines); lc++) {
			for (int lb = 0; lb < numOfColumns + 1; lb++) {
				...
				if (lb == 0) {
					...
					longBuf = Double.parseDouble(strBuf); // called longBuf
															// because
					lineRefs[lc - numOfBarSeries][lb - 1] = longBuf; // was
																		// originally
																		// type
																		// long
...
class ChartPanel extends JPanel { // this panel draws an image of the chart
									// object passed to it... it is itself the
									// key output.

	private static final long serialVersionUID = 5231574239848039300L;
	// KEY INPUTS TO THIS CLASS:
	ChartColourScheme chartColors; // colour scheme object to allow different
									// colours easily
	Chart theChart; // Contains all the data required for the paint(g)

	// DATA FROM CHART:
	int[] xAreaPoints; // the x-co-ords of points used to draw area graph (if
						// required)
	int numOfBars;
	int numOfBarSeries;
	int numOfBarSets; // the # of series along the x-axis (therefore # of
						// labels)
	...
	// CALCULATED FIELDS:
	int panelHeight; // used to dynamically resize the chart everytime paint(g)
						// is called
	int panelWidth;
	...
	int xRightIndent; // to allow for more space on RHS if a secondary axis is
						// required
	int currentFontSize; // for the purposes of adjusting the font if it is too
							// big
	int drawnBars;
	int chartHeight;
	int chartWidth;
	...
	// Parameter für farblich hinterlegte Anzeige der Buchung und Beschriftung
	int buchung;
	int dauer;
	...
	public ChartPanel(ChartColourScheme col, Chart ch, int buch, int dau,
			String labelAlt, String labelBuch) {
		chartColors = col;
		theChart = ch;
		numOfBars = theChart.getNumOfBars();
		numOfBarSeries = theChart.getNumOfBarSeries();
	...
	public void paint(Graphics gr) {
		...
		setGaps(panelHeight, panelWidth); // determine gaps depending on size of
											// panel

		chartHeight = panelHeight - 2 * yIndent; // determing the height & width
													// of the piece
		chartWidth = panelWidth - (xIndent + xRightIndent); // of the panel we
															// draw the chart
															// in.
		...
		preparePanel();
		drawPrimaryAxis();
		if (theChart.areaForChart())
			drawArea();
		drawBars();
		if (theChart.showXLabels())
			drawXAxisLabels();
		if (theChart.showYLabels())
			drawYAxisLabels();
		if (theChart.showMaxLabels())
			drawMaxMinLabels();
		if (theChart.linesForChart())
			drawTheLines();
	...
	public Font getIdealFont(int c) // c is an adjuster - minsued from
									// calculated
	{ // font size
	...
	public void setGaps(int hgt, int wdth) {
		...
		// the greater the number of bars to be drawn the smaller barGap needs
		// to be...

		barGap = (int) (2 * (xIndent / numOfBars) + 1);

		if (barGap > 6)
			barGap = 6; // maximum gap is 6
	...
	public void preparePanel() {
		...
		g.fillRect(barGap, barGap / 2, panelWidth - 2 * barGap, panelHeight
				- barGap);
	}

	public void drawPrimaryAxis() {
		g.setColor(chartColors.getAxisColor());
		g.drawLine(xIndent - barGap, yIndent, xIndent - barGap, panelHeight
				- yIndent + barGap);
		g.drawLine(xIndent - barGap, panelHeight - yIndent + barGap, panelWidth
				- xRightIndent + barGap, panelHeight - yIndent + barGap);
		// g.drawLine(panelWidth - xIndent + barGap, yIndent, panelWidth -
		// xIndent + barGap, panelHeight - yIndent + barGap);
		// now the axes have been drawn - secondary axis is drawn if required
		// (check chart object)
	}

	public void drawArea() {
		g.setColor(chartColors.getAreaColor());
		int[] yAreaPoints = new int[numOfBarSets + 3]; // to determine
														// y-coords...
		double yAreaPtBuf;

		for (int d = 1; d < numOfBarSets + 1; d++) {
			yAreaPtBuf = ((pRange - (theChart.getAreaRef(d) - pMin)))
					* chartHeight / pRange + yIndent;
	...
	public void drawBars() {
		...
		extraBarGap = 0;
		String labelText;
		int xDrawPoint = xIndent; // records how far across the x-axis we have
									// gone.
		int zaehler = 0;
		...
		for (int a = 0; a < numOfBars; a++) {
			double num = theChart.getBarRef(a); // bar ref is a % scores showing
												// how far up
			double yPerc = (double) ((pRange - (num - pMin)) / pRange * chartHeight); // the
																						// axis
																						// the
																						// point
																						// is

			int yCoord = (int) (yPerc); // this is converted into a proportion
										// of chartheight
			...
			g.drawString(labelText, xDrawPoint + addSpace,
					(panelHeight - (xIndent + barGap) / 2));

			xDrawPoint += extraBarGap; // increment for the next bar to be drawn

			// Zusatz für Darstellung der Buchung
			if (schalter) {
			...
			g.fillRect(xDrawPoint, yCoord + yIndent, barWidth - barGap,
					(chartHeight - yCoord));

			if (theChart.drawBarFrames()) {
				g.setColor(Color.black);
				g.drawRect(xDrawPoint, yCoord + yIndent, barWidth - barGap,
						(chartHeight - yCoord));
			}

			extraBarGap = 0; // so that a larger gap appears between x-axis
								// groups
			xDrawPoint += barWidth;
			...
			if (curBarSeries > numOfBarSeries) // implies new column
			{
				...
				extraBarGap = 2 * barGap; // larger gap for 1st element of new
											// group

				g.setColor(chartColors.getAxisColor()); // to draw line
														// seperating x-axis
														// groups

				if (!(curBarSet > numOfBarSets - 1))
					g.drawLine(xDrawPoint + (int) (barGap / 2), panelHeight
							- yIndent, xDrawPoint
							+ (int) (3 * barGap / 2 - barGap), panelHeight
							- yIndent + 2 * barGap);
			}
		}
	}
	...
	public void drawXAxisLabels() {
		...
		for (int k = 0; k < numOfBarSets; k++) {
			// Zusatz für Darstellung der dynamischen Beschriftungen
			if (schalter) {
				g.setColor(Color.blue);
				g.drawString(theChart.getColName(1), xTextInsert, yTextInsert);
			} else {
				g.drawString(theChart.getColHeading(k), xTextInsert,
						yTextInsert);
			}
	...
	public void drawMaxMinLabels() {
		...
		g.drawString(maxString, xIndent - 2 * barGap
				- (maxString.length() * 2 * currentFontSize / 4), yIndent + 2
				* barGap);
		g.drawString(minString, xIndent - 2 * barGap
				- (minString.length() * 2 * currentFontSize / 4), chartHeight
				+ yIndent);
		// g.drawString("test", xIndent - 2*barGap - (4*2*currentFontSize/4) ,
		// highBelegung);
	}
	...
	public void drawTheLines() {
		...
		int xPos = 0;
		int xLastPos = 0;
		int xIncrease = (numOfBarSeries * barWidth) + 2 * barGap; // lines
																	// always
																	// drawn at
		...
		for (int f = 0; f < theChart.getNumOfLines(); f++) // centre of series
		{
			...
			for (int h = 0; h < numOfBarSets; h++) {
				...
				yPos = (int) (yPer);
				// as is done for bars and areas the value stored in chart is
				// extracted - then the % of the range
				// that this value contains is calculated. This % is then used
				// to determine how far up the y-axis
				...
				{
					g.drawLine(xLastPos - barGap / 2 + 1, yLastPos + yIndent,
							xPos - barGap / 2 + 1, yPos + yIndent);
					g.drawLine(xLastPos - barGap / 2, yLastPos + yIndent, xPos
							- barGap / 2, yPos + yIndent);
					g.fillOval(xLastPos - 3 * barGap / 2 + 1, yLastPos
							+ yIndent - barGap + 1, 2 * barGap, 2 * barGap);
			...
			g.fillOval(xLastPos - 3 * barGap / 2 + 1, yLastPos + yIndent
					- barGap + 1, 2 * barGap, 2 * barGap);
		}
	}
	...
	public void drawSeriesLabels() {
		...
		for (int q = 0; q < numSeries; q++) {
			...
			if (q < numOfBarSeries) {
				...
				g.fillRect(labelInPoint + barGap / 2, 2 * yIndent / 3 - 2
						* barGap, 2 * barGap, 2 * barGap);
			} else if (theChart.areaForChart()
					& (q == (theChart.getNumberOfSeries() - 1))) {
				g.setColor(chartColors.getAreaColor());
				...
				g.drawLine(labelInPoint + barGap / 2, 2 * yIndent / 3 - barGap,
						labelInPoint + barGap / 2 + 2 * barGap, 2 * yIndent / 3
								- barGap);
			}
