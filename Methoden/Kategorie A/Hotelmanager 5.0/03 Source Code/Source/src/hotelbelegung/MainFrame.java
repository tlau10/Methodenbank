...
import hotelbelegung.model.Settings;
...
public class MainFrame extends JFrame {
    ...
    private DefaultTableModel buchungenModel;
    ...
    private int Saison_von = 0; // 01.Januar
    ...
    private void init() throws Exception {
        ...
        this.setTitle("Hotel-Manager 5.0");
        ...
        jMenuHelpAbout.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                jMenuHelpInfo_actionPerformed(e, topFrame);
            }
        });
        ...
        arbeitsVerzeichnis = settings.tempPath;
        ...
        RegisterJTabbedPane.addTab("Reservierung", null, ReservDatumJPanel,
                null);
        RegisterJTabbedPane.addTab("Buchungen", null, BuchungenPanel, null);
    ...
    private void createBuchungenPanel() {
        JLabel AnreisedatumJLabel = new JLabel();
        AnreisedatumJLabel.setText("Aktuelle Buchungen:");
        AnreisedatumJLabel.setBounds(new Rectangle(10, 10, 120, 29));

        String[] columns = { "Start-Datum", "Anzahl Tage", "Gesamtpreis" };
        List<Buchung> buchungen = myManager.getBuchungStore().getBuchungen();

        buchungenModel = new DefaultTableModel(columns, 0);

        for (Buchung b : buchungen) {
            displayNeueBuchung(b);
        }

        JTable buchungJTable = new JTable(buchungenModel);
        ...
        deleteButton.addActionListener(new ActionListener() {
            ...
            @Override
            public void actionPerformed(ActionEvent e) {
                List<Buchung> toBeRemovedFromModel = new ArrayList<>();
                for (int i = 0; i < buchungJTable.getSelectedRowCount(); i++) {
                    int selectedRow = buchungJTable.getSelectedRow();
                    buchungenModel.removeRow(selectedRow);
                    toBeRemovedFromModel.add(buchungen.get(selectedRow));
                }
                buchungen.removeAll(toBeRemovedFromModel);
            }
        });
        ...
        saveButton.addActionListener(new ActionListener() {
            ...
            @Override
            public void actionPerformed(ActionEvent e) {
                myManager.getBuchungStore().speichern();
            }
        });

        this.add(saveButton);

        // BuchungenJPanel-Panel hinzufügen
        BuchungenPanel = new JPanel();
        BuchungenPanel.setLayout(new BoxLayout(BuchungenPanel, BoxLayout.Y_AXIS));
        BuchungenPanel.setBackground(new Color(212, 225, 223));

        BuchungenPanel.add(buchungJTable);
        BuchungenPanel.add(deleteButton);
        BuchungenPanel.add(saveButton);
    }

    /** Aktion Hilfe | Info durchgeführt */
    protected void jMenuHelpInfo_actionPerformed(ActionEvent e, Frame f) {
        JDialog helpDialog = new JDialog(f, "Information", true);

        JLabel infoLabel = new JLabel();
        infoLabel.setText(
                "<html>1.Wählen Sie die Anzahl der Tage aus genau so wie die Zimmerkategorie.<br/>"
                        +
                        "2. Stellen Sie sicher das der Solver Richtig eingebunden wurde.<br/>" +
                        "(meistens wird das backslash vergessen)<br/>"
                        +
                        "3. Sobald man bucht erscheint ein Fenster das frägt ob man das Aktuelle Datum beibehalten möchte oder das " 
                        + "unten stehende Alternative Termin nimmt. Falls Aktuelles Termin dann Annhemen klicken ansonsten Ablehnen.<br/>" +
                        "(solang machen bis man sein Wunschtermin hat) <br/>"
                        +
                        "4. Nun kann man nach dem Buchen in dem Reiter Buchungen die Termine Speichern und löschen. <br/>" +
                        "(gespeichert wird in der buchungen.dat detei) <br/>"
                        +
                        "5. Bei weiteren Fragen Hilft Ihnen die Dokumentation weiter. <br/>" +
                        "</html>");
        helpDialog.add(infoLabel);
    ...
    void AnfrageStartenJButton_actionPerformed(ActionEvent e) {
         ...
         *
         * String, welcher den Pfad für die hotel.out Datei zeigt. Die TextArea wird mittels RegisterJTabbedPane.add()
         * dem Frame hinzugefügt.
         *
         * Dann wird genau so weiter gemacht, wie für die hotel.lp Datei.
         *
         **********************************************************************************************/

        String hotelLP = settings.tempPath + "hotel.lp";
        String holtelOUT = settings.tempPath + "hotel.out";
        ...
        try {
            ...
            if (korrekteWerte) {
                ...
                arbeitsVerzeichnis = this.ArbeitsverzeichnisJTextField
                        .getText();
                settings.tempPath = arbeitsVerzeichnis;

                solverPfad = this.SolverpfadJTextField.getText();
                settings.lpPath = solverPfad;
    ...
    public void lpSolveScript() {
        ...
        String komplScript = laden.Laden(settings.tempPath + "hotel.out");
    ...
    public void displayNeueBuchung(Buchung b) {
        buchungenModel.addRow(new Object[]{ b.startDatum, Integer.toString(b.anzahlTage), Integer.toString(b.gesamtPreis) });
    }
}
