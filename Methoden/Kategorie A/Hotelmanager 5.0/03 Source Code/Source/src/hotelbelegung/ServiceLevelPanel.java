...
public class ServiceLevelPanel extends JPanel {
	...
	private int chartBars = 10; // Standard Anzeige der Belegung über zehn Tage
								// hinweg
	...
	// Normalansicht
	public ServiceLevelPanel(Manager manager, Belegung belegung,
			int maxBelegung, Date beginnAnzeige, int tageAnzeige, int kategorie) {
		try {
			jbInit(manager, belegung, maxBelegung, beginnAnzeige, tageAnzeige,
					kategorie);
	...
	public ServiceLevelPanel(Manager manager, Belegung belegung,
			int maxBelegung, Date altDatum, Date buchung, int anzTage,
			int kategorie) {
		try {
	...
	private void jbInit(Manager manager, Belegung belegung, int maxBelegung,
			Date beginnAnzeige, int tageAnzeige, int kategorie)
			throws Exception {
		maximum = maxBelegung;
		...
		chart = new Chart(jTableServiceLevel, chartCols, chartBars, 0, false,
				maximum, minimum, true, false);
	...
	private void jbInit(Manager manager, Belegung belegung, int maxBelegung,
			Date altDatum, Date buchung, int anzTage, int kategorie)
			throws Exception {
		maximum = maxBelegung;
		this.setLayout(null);
		this.setMinimumSize(new Dimension(251, 108));
		...
		chartBars = anzTage * 3; // Anzeige des gesamten
									// Alternativbuchungs-Zeitraums
		...
		for (int i = 1; i <= chartBars; i++) {
			...
			int vglBuchDatum = Integer
					.parseInt(compareFormat.format(buchDatum));
		...
		chartPan = chart.getPanel(tagBuchung, anzTage, bezAlternative,
				bezBuchung);
