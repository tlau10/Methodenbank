...
/**
 * Implementiert den Parser fuer LP_SOLVE.EXE, wobei hier nur Primalloesungen
 * geparst werden koennen. Der Aufruf darf keine Optionsparameter enthalten,
 ...
 */
  ...
  */
  ...
  /**
  * Gibt Optimumswert zurueck. Voraussetzung dafuer ist, dass eine Loesung
 ...
 /**
  * Ermittelt (Variable=Wert) Tupel vom Objekttyp String (Primalloesungen).
  * Voraussetzung dafuer ist, dass eine Loesung existiert, d.h. der Solver
  ...
  */
  ...
  private int setztErgebnisVektoren()
  {
    ...
    //System.out.println("helpStr nach befüllung:  " + helpStr[0] );
    ...
    // Pruefen ob Optimum da..
    ermittleZahl(helpStr);
    try
    {
        //System.out.println("Optimum ?:" + Double.valueOf(helpStr[0]));
        _optimum = Double.valueOf(helpStr[0]);
  ...
  private int ermittleZahl(String[] doubleStr)
  {
    ...
    {
      helpStr += _token;
      //System.out.println("Methode ermittleZahl() helpStr: " + helpStr);
      if (!finished()) _token = getNextElement();
      else break;
    }
...
//  System.out.println("Schreibe in doubleString");
    doubleStr[0] = helpStr;
    _errMsg = "Kein Fehler";
    return 0;
  }
  ...
  private int macheAuswertung()
  {
    ...
    while (!finished())
    {
      ...
      //Wenn Variable und Ergebniss vorhanden -> Fuege Ergebnistupel in Vector ein!
      ...
      else
      {
        if (ergebnisStr[0] == "")
        {
          _errMsg = "Kein gueltiges Ergebnis!\r\n";
          _errMsg += COMMON_ERROR_MSG1;
          return -4;
        }
        if (variableStr == "")
        {
          _errMsg = "Keine gueltige Variable!\r\n";
          _errMsg += "-> Syntax:  <Variable>::=[Buchstabe]{Ziffer}\r\n";
          _errMsg += COMMON_ERROR_MSG1;
          return -5;
        }
