...
//* 	von Francis Gaeltner, Helmut Lindinger, Bernd Saile			*
//*																	*
//*		Synopsis: Enthaelt die Klasse fuer die Fenster der Eingabe-	*
...
public class plo_Eingabemaske extends JInternalFrame{
	...
	final JLabel em_NachfrageNr1;									//Beschriftungskomponenten fuer
...
//*** Konstruktor  ***------------------------------------------------
	public plo_Eingabemaske(plo_Hauptfenster base, int anzNach, String title, boolean resizable, boolean maximizable, boolean closable, boolean iconifiable)
	{
		...
		plo_EingabePanel.add(em_NachfrageNr1);						//Hinzufuegen der Komponenten zum Panel
		...
		{															//Anzahl, die vom Benutzer eingegeben worden ist.
			...
			//Setzen der Nachfragennummer und Perioode aufsteigend
			...
			this.buildConstraints(em_GridBagConstraints, em_nach.getTf_Nummer(), 0, i, 2, 1, GridBagConstraints.NONE, GridBagConstraints.NORTHWEST, 5, 5, standard);							//Erstellen der Constraints fuer die Textfelder
			...
			plo_EingabePanel.add(em_nach.getTf_Nummer());			//Hinzufuegen der Textfelder
		...
		this.getContentPane().add(em_EingabeScrollPane = new JScrollPane(plo_EingabePanel));	//Hinzufuegen des Scrollpanes mit dem Darstellungspanels
		this.addInternalFrameListener(em_Listener);												//Hinzufuegen des Listeners
	}
//*** Ende Konstruktor  ***-------------------------------------------
...
//*** Alternativer Konstruktor  ***-----------------------------------
	...
	{																//Alternativer Konstruktor, fuer den Fall, dass ein vorhandenes Modell, mit Name und bereits vorhandener
		...
		em_IsOptimized = false;												//
		em_ZugehoerigesErgebnisfenster = new plo_Ergebnisfenster(base, title+" - Ergebnis", false, false, true, true, this);	//Initialisieren des Referenzobjekts
		em_ZugehoerigesErgebnisfenster.setVisible(true);																		//
		...
		this.buildConstraints(em_GridBagConstraints, em_NachfrageNr1, 0, 0, 2, 1, GridBagConstraints.NONE, GridBagConstraints.WEST, 5, 0, new Insets(0,5,0,5));			//Erstellen der Constraints fuer die
		...
		plo_EingabePanel.add(em_NachfrageNr1);						//Hinzufuegen der Komponenten zum
		...
		em_NachfragenListe = new nachfrage[anzahlNachfragen];		//Kopieren des uebergebenen Nachfragen-Arrays in
		...
		{															//
			...
			this.buildConstraints(em_GridBagConstraints, this.getEm_NachfragenListe(i).getTf_Nummer(), 0, i+2, 2, 1, GridBagConstraints.NONE, GridBagConstraints.NORTHWEST, 5, 5, standard);							//Erstellen der Constraints fuer die
			...
			plo_EingabePanel.add(this.getEm_NachfragenListe(i).getTf_Nummer());			//Hinzufuegen der Textfelder
		...
		this.getContentPane().add(em_EingabeScrollPane = new JScrollPane(plo_EingabePanel));	//Hinzufuegen des ScrollPanes mit dem Darstellungspanel
		this.addInternalFrameListener(em_Listener);												//Hinzufuegen des Listeners
	}
...
//*** SET-Methoden  ***-----------------------------------------------
	...
	public void setEm_ZugehoerigesErgebnisfenster(plo_Ergebnisfenster elem)	//Methode zum Setzen des Referenzobjektes auf das Ergebnisfenster des Modells
	{
...
//*** GET-Methoden  ***-----------------------------------------------
	public boolean getEm_IsSaved()									//Methode zur Rueckgabe des boolschen isSaved-Wertes
	{
		return em_IsSaved;
	}

	public boolean getEm_IsOptimized()								//Methode zur Rueckgabe des boolschen isOptimized-Wertes
	{
		return em_IsOptimized;
	}

	public plo_Ergebnisfenster getEm_ZugehoerigesErgebnisfenster()	//Methode zur Rueckgabe des Referenzobjektes auf das Ergebnisfenster des Modells
	{
		return em_ZugehoerigesErgebnisfenster;
	}

	public int getAnzahlNachfragen()								//Methode zur Rueckgabe der Nachfragenanzahl
	{
	...
	public nachfrage getEm_NachfragenListe(int index)				//Methode zur Rueckgabe der Nachfrage in der Nachfragenliste an einem Index
	{
	...
	public plo_Matrix getEm_Core()									//Methode zur Rueckgabe der LP-Matrix des Modells
	{
		return em_Core;
	}

	public int getFensterIndex()									//Methode zur Rueckgabe des Fensterindexes
	{
		return fensterIndex;
	}
//*** Ende GET-Methoden  ***------------------------------------------
...
//*** Methode zum Abspeichern der Daten  ***--------------------------
	...
	{																//unter einem durch Dateidialog gewonnenen Pfad und Dateinamen
		...
		try
		{
			BufferedWriter w = new BufferedWriter(new FileWriter(dateiname));	//oeffnen bzw. Erstellen der
...
//*** Methode zum Laden einer Textdatei  ***-------------------------------
	...
	{																	//mittels eines durch einen Dateidialog ermittelten Dateinamens
		...
		{																//
			...
			BufferedReader r = new BufferedReader(new FileReader(dateiname));	//oeffnen der Datei mittels des
...
//*** Methode zum Einfuegen einer weiteren Nachfrage  ***--------------
	public void nachfrageEinfuegen()									//Methode um am Ende des vorhandenen Modells eine
	{																//neue Nachfrage anzuhaengen
		nachfrage[] tempListe = new nachfrage[anzahlNachfragen+1];	//Erstellen eines neuen, um 1 laengeren Nachfrage-Arrays
		...
		{															//so dass die letzte Nachfrage leer bleibt
		...
		for(int i = 0; i < anzahlNachfragen; i++)					//mit einer zusaetzlichen Nachfrage. Danach werden die Nachfragen aus
		...
		this.getEm_ZugehoerigesErgebnisfenster().dispose();			//Schliessen des Ergebnisfensters
		this.dispose();												//Schliessen des alten Modells
	}
...
//*** Methode zum Entfernen einer bestimmten Nachfrage  ***--------------
	...
	{																//Nachfrage aus der Liste des Modells
		...
		{															//Index+1 auf die Nachfrage an der Stelle Index, beginnend, mit dem Index, der geloescht werden soll
			em_NachfragenListe[i] = em_NachfragenListe[i+1];		//
		}															//
		em_NachfragenListe[anzahlNachfragen-1] = null;				//Loeschen der letzten Nachfrage in der Liste und
		...
		this.getEm_ZugehoerigesErgebnisfenster().dispose();			//Schliessen des verbundenen Ergebnisfensters
		this.dispose();												//Schliessen des alten Modells
	}
...
//*** Methoden zum Erzeugen der Matrix***------------------------------
	...
	{																//in der zugehoerigen Matrix
		int max = 0;
		int spaltenzahl = 0;
		int zeilenzahl = 0;
		boolean isSet = false;										//Indikatorvariable um anzuzeigen, ob ein Matrixstring veraendert wurde
		Integer intTemp;
		Double doubleTemp;
		String buffer = new String("");
		//Ermittlung der PeriodenAnzahl
		for(int i = 0; i < this.getAnzahlNachfragen();i++)			//Ermittlung der noetigen Periodenzahl durch Gewinnung der
		{															//Nachfrage mit der hoechsten Periodenzahl
		...
		zeilenzahl = ((2 * max) +1);								//Berechnung der Zeilenzahl: 1x max (fuer das System mit den Schaltervariablen der Lagerkosten), 1x max (fuer das System mit den Schaltervariablen der Bestellkosten),
																	// und 1 fuer diee Zielfunktion.
...
*/
		...
		for(int i = 0; (i < max) && (arrayCounter < this.getAnzahlNachfragen()); i++)	//Auffuellen der Stellen im Gesamtperiodenarray, in denen keine Nachfrage statt
		{																				//findet mit 0
			if(nextNach == nextVal)
			{
...
//*** Methode zu Umwandlung der Matrix ***----------------------------
	public String[] matrixToLpModell()					//Methode um die Matrix in ein String-Array zu ueberfuehren
	{
