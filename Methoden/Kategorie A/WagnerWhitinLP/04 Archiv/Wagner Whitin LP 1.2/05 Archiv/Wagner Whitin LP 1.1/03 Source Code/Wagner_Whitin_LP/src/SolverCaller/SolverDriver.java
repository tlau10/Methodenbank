...
/**
 * Das Programm ist der Treiber fuer das Solver Aufruf Packet. Ein Client Programm
...
public class SolverDriver implements IErrorMessage
{
   ...
   * in Klasse BatchSolver muss in der verwendeteten Solverklasse realisiert
   * werden.
  ...
  */
   ...
   * des Models muss dem des verwendeten Solvers entsprechen. 
   * @param solver Der Solver der verwendet wird
   * @param anzLPZeilen Anzahl der erwarteten LP Zeilen
   * @param lpZeilen Ein Stringfeld mit den LP-Zeilen
   */
   public SolverDriver( BatchSolver solver,int anzLPZeilen, String[] lpZeilen){
    // Solver und Parser anlegen..
    this.solver = solver;
  ...
  * die zugehoerigen Werte (Objekttyp Double).
  */
  public Vector[] getPrimalResult()   { 
	  return primalErgebnis; 
  }
  ...
  */
  ...
  // Private Methoden
  //-----------------------------------------------------------------------
  // Setzt die Rueckgabewert wie z.B. Optimum, Primalergebnisse usw.
  private void starteAuswertung(int errorcode)
  {
    // Solveraufruf auf Fehler pruefen..
    if (errorcode >= 0)
     {  // Kein Fehler. Variablen / Loesungen auslesen...
        if (errorcode == 0)
        {
