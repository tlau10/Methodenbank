 ...
 *                   Richard Atkinson;
 *
 * $Id: AreaXYItemRenderer.java,v 1.5 2002/10/11 11:27:42 mungady Exp $
 *
 * Changes:
 * --------
...
public class AreaXYItemRenderer extends AbstractXYItemRenderer implements XYItemRenderer {
    ...
    public void drawItem(Graphics2D g2, Rectangle2D dataArea,
        ...
        double transY2;
        boolean infinitiy = false;

        if (item == 0) {
            // Create a new Area for the series
            pArea = new Polygon();

            if(x1.equals(data.getXValue(series, item+1)) && y1.equals(data.getYValue(series, item+1))) {
              infinitiy = true;
            }

            if(!infinitiy) {
              // start from Y = 0
              transY2 = verticalAxis.translateValueToJava2D(0.0, dataArea);
            }
            else {
              transY2 = verticalAxis.translateValueToJava2D(100.0, dataArea);
            }
    ...
    public void drawSolutionArea(Graphics2D g2, Rectangle2D dataArea,
                            ChartRenderingInfo info,
                            XYPlot plot, ValueAxis horizontalAxis, ValueAxis verticalAxis,
                            XYDataset data, int series, int item,
                            CrosshairInfo crosshairInfo) {
           ...
           // If there is only one item in series: return
           if (itemCount < 1) {
             return;
           }
           ...
           // Check, if solution area goes to x1-infinity (that is when very
           // last point and the point before are exactly the same)
           if (item > 1  && item == (itemCount-1)) {
             if (x1.equals(data.getXValue(series, item-1)) && y1.equals(data.getYValue(series, item-1))) {
               float xDiff = (data.getXValue(series, item-1)).floatValue()-(data.getXValue(series, item-2)).floatValue();
               float yDiff = (data.getYValue(series, item-1)).floatValue()-(data.getYValue(series, item-2)).floatValue();
               transX1 = horizontalAxis.translateValueToJava2D((x1.doubleValue())+(xDiff*100), dataArea);
               double tmptrans = verticalAxis.translateValueToJava2D(0, dataArea);
               transY1 = verticalAxis.translateValueToJava2D((y1.doubleValue())+(yDiff*100), dataArea);
               if (transY1 > tmptrans) {
                transY1 = tmptrans;
               }
             }
           ...
           if (item > 1  && item == (itemCount-1)) {
             if (x1.equals(data.getXValue(series, item-1)) && y1.equals(data.getYValue(series, item-1))) {
               transY1 = verticalAxis.translateValueToJava2D(0, dataArea);
               pArea.addPoint((int) transX1, (int) transY1);
             }
           }
           ...
           if (item > 0 && item == (itemCount - 1)) {
               if ((data.getXValue(series, 0)).equals(data.getXValue(series, 1))
                && (data.getYValue(series, 0)).equals(data.getYValue(series, 1))) {
                 // if polygon should be drawn into y-infinity
                 transY2 = verticalAxis.translateValueToJava2D(100.0, dataArea);
                 pArea.addPoint((int) transX1, (int) transY2);
               }

               // fill the polygon
               if (itemCount >= 4 && (pArea.xpoints[0] == pArea.xpoints[1]) &&
                  (pArea.ypoints[pArea.npoints-1] == pArea.ypoints[pArea.npoints-2])) {
                  sortPolygon(pArea);
                  polySwap(pArea, 0,1);
               }
               else if (itemCount >= 4 && (pArea.xpoints[0] == pArea.xpoints[1])) {
                  sortPolygon(pArea);
               }
    ...
    public void sortPolygon(Polygon polygon) {
      int i;
      int j;
      for (i = 1; i < polygon.npoints; i++) {
        for (j = polygon.npoints-1; j >= i; j--) {
          if(polygon.xpoints[j-1] > polygon.xpoints[j]) {
            polySwap(polygon, j, j-1);
          }
          if (polygon.xpoints[j-1] == polygon.xpoints[j]) {
            if (j == polygon.npoints-1 && polygon.ypoints[j] >= 0) {
              if (polygon.ypoints[j] == polygon.ypoints[j-1]) {
                if (polygon.xpoints[j-1] == polygon.xpoints[j-2]) {
                  if (polygon.ypoints[j-1] < polygon.ypoints[j-2]) {
                    polySwap(polygon, j-1, j-2);
                  }
                }
              }
              else {
            ...
            if (j == 1 && polygon.ypoints[j-1] >= 0) {
              if (polygon.ypoints[j-1] < polygon.ypoints[j]) {
                polySwap(polygon, j, j-1);
              }
            }
          }
    ...
    public void polySwap(Polygon poly, int i1, int i2)
    {
      int tmpx = poly.xpoints[i1];
      int tmpy = poly.ypoints[i1];
      poly.xpoints[i1] = poly.xpoints[i2];
      poly.ypoints[i1] = poly.ypoints[i2];
      poly.xpoints[i2] = tmpx;
      poly.ypoints[i2] = tmpy;
      return;
    }
