 ...
 *                   Jonathan Nash;
 *
 * $Id: XYPlot.java,v 1.10 2002/10/16 15:44:06 mungady Exp $
 *
...
public class XYPlot extends Plot implements HorizontalValuePlot,
    ...
    public void render(Graphics2D g2, Rectangle2D dataArea,
        ...
        if (data != null) {
            ...
            int seriesCount = data.getSeriesCount();
            boolean lastSerie = false;
            for (int series = 0; series < seriesCount; series++) {
                int itemCount = data.getItemCount(series);
                if(series == seriesCount-2) {
                  lastSerie = true;
                }
                for (int item = 0; item < itemCount; item++) {
                  if(!lastSerie) {
                    ...
                    AreaXYItemRenderer rendererArea = (AreaXYItemRenderer)renderer;
                    rendererArea.drawSolutionArea(g2, dataArea, info, this,
                                        domainAxis, rangeAxis,
                                        data, series, item,
                                        crosshairInfo);
                  }
