...
import iterator.gui.javafx.BasicWindowController;
...
public class LogicSimplexRational implements ILogic {
	...
	public TableauCellPosition pivot(ITableau tableau) throws Exception {
		...
		if(pivotColumn == -1){
					...
					logger.info("  >>OPTI pivot column selection criterion injured!" );
					BasicWindowController.startAlertOptimum();
		...
		for (int i = 0; i < bVektor.length; i++) {
			...
			//existieren positive Werte in Pivot Spalte
			if((bVektor[i].isPositive() || bVektor[i].isZero()) &&
					aMatrix[pivotColumn][i].isPositive() && bVektor[i].divide(aMatrix[pivotColumn][i]).isLessThan(smallestValue)){
		...
		if(pivotRow == -1){
					...
					BasicWindowController.startAlertUnboundedSolution();
