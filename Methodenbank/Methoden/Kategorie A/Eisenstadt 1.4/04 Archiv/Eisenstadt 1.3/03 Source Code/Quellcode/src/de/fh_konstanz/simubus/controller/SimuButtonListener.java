   ...
   public void mouseClicked( MouseEvent evt )
   {
      ...
      if ( actionCmd.equals( "addBuslinie" ) )
      {
         ...
         Linie linie = new Linie( "Linie "
               + String.valueOf( Strassennetz.getInstance().getArrayBuslinie().size() + 1 ) );
         ...
         if ( !strassennetz.getArrayBuslinie().isEmpty() && strassennetz.getAlleHaltestellen().size() >= 2 )
         {
            Thread t = new Thread( new Runnable()
            {
               ...
               public void run()
               {
                  ...
                  final Experiment exp = new Experiment( "Bus Simulations Experiment", p, panel
                        .getSelectedStartpunkt(), 2, "desmoj.core.report.HTMLReportOutput",
                  ...
                  progress.setIconImage( new ToolkitImage( new URLImageSource( ImageUtil
                  ...
                  progress.addWindowListener( new WindowAdapter()
                  {
                     ...
                     {
                        if ( JOptionPane.showOptionDialog( progress,
                              "Wollen Sie Sie die Simulation abbrechen?", "Abbrechen",
...
//                stopButton ActionListener
      ...
      else if ( actionCmd.equals( "reportsSimu" ) )
      {
         ...
         {
            ReportFrame report = ReportFrame.getInstance( View.getInstance().getBounds() );
            report.createReportHierarchie();
