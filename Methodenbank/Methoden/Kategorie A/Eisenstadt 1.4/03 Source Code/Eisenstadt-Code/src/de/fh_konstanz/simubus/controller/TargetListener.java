...
/**
 * Die Klasse <code>TargetListener</code> reagiert darauf, falls ein Ziel in das
 * Modell eingefuegt, verschoben oder geloescht wird.
 * 
					...
					if (changed[i] instanceof ZieleCell) {
						...
						for (int j = 0; j < planquadrate.length; j++) {
							for (int k = 0; k < planquadrate[j].length; k++) {
								Rectangle2D rec = GraphConstants.getBounds(((ZieleCell) changed[i]).getAttributes());

								Rectangle2D.Double r = planquadrate[j][k].getBounds();

								if (r.contains(new Point2D.Double(rec.getCenterX(), rec.getCenterY()))) {
									Ziel ziel = new Ziel(planquadrate[j][k].getPlanquadratCoordinates(), z.getId());
