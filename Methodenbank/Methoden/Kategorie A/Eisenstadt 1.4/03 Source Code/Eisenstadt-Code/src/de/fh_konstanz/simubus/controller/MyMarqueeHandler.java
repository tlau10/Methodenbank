...
public class MyMarqueeHandler extends BasicMarqueeHandler {
	...
	public void mousePressed(final MouseEvent e) {
		...
		if (SwingUtilities.isLeftMouseButton(e)) {
			if (port != null && graph.isPortsVisible()) {
				...
				// JTabbedPane linien = findJPanelWithName(simuControlPanel,
				// "simuControlPanel");

				JToggleButton zieleEinfuegen = findJToggleButtonWithName(View.getInstance().getContentPane(),
						"Ziele einfügen");
				JToggleButton haltestellenEinfuegen = findJToggleButtonWithName(View.getInstance().getContentPane(),
						"Haltestelle einfügen");
				JToggleButton strasseEinfuegen = findJToggleButtonWithName(View.getInstance().getContentPane(),
						"Strasse einfügen");
				JToggleButton strasseGesperrt = findJToggleButtonWithName(View.getInstance().getContentPane(),
						"gesperrte Strasse einfügen");
				JToggleButton halteStellegesperrt = findJToggleButtonWithName(View.getInstance().getContentPane(),
						"gesperrte Haltestelle einfügen");
				JToggleButton loeschen = findJToggleButtonWithName(View.getInstance().getContentPane(), "loeschen");

				try {
					// if (simuPanel.isVisible() == true &&
					// linien.getSelectedIndex()!=1){
					// if (true){
					...
					Logger.getInstance().log(
							"Fehler bei der Durchsuchung des Panels nach JToggleButtons" + " in MyMargueeHandler,java");
				}
			}
		}
		if (SwingUtilities.isRightMouseButton(e)) {
	...
	public JToggleButton findJToggleButtonWithName(Container container, String jToggleButtonText) {
		...
		for (int i = 0; i < components.length; i++) {
			...
			// if the component is a container, find out if it contains the
			// JButton
	...
	// public JTabbedPane findJPanelWithName(Container container, String
	// JPanelText) {
	// Component[] components = container.getComponents();
	//
