...
class ChartColourScheme {
	...
	private Vector<Color> barColors;
	private Vector<Color> lineColors;
	...
	public ChartColourScheme(int bars, int lns, int ars) {
		...
		barColors = new Vector<Color>(); // we will populate this with default
											// colours
		for (int a = 0; a < 3; a++) {
			barColors.add(Color.black); // although the size of this array
			barColors.add(Color.darkGray); // is dynamic for convenience
			barColors.add(Color.gray); // we make 18 entries - enough for most
										// requirements.
			barColors.add(Color.blue);
			barColors.add(Color.cyan);
			barColors.add(Color.magenta);
			barColors.add(Color.red);
			barColors.add(Color.orange);
			barColors.add(Color.yellow);
			barColors.add(Color.white);
		}

		lineColors = new Vector<Color>(); // similarly we will construct a large
											// number for safety
		for (int b = 0; b < 3; b++) {
	...
	public void setChartAreaColor(int r, int g, int b) {
		chartAreaColor = new Color(r, g, b);
	}
	...
	public Color getLineColor(int a) {
		if (a > (lineColors.size() - 1))
			a = 0; // a check to avoid a crash if the Chart instance
		Color colBuf = (lineColors.get(a)); // asks for a colour beyond the
											// bound of the
	...
	{
		if (serNum > (barColors.size() - 1))
			serNum = 0;
		Color colBuf = (barColors.get(serNum));
		return colBuf;
	}
...
class ColorPanel extends JPanel implements ActionListener, ChangeListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -8553724381305438296L;
	// a convenient device for changing the colours stored within the
	// ChartColorSchemem instance.
	private JPanel colPan;
	private ChartColourScheme colScheme;
	private JComboBox<String> chartElements;
	...
	public ColorPanel(ChartColourScheme colSch) {
		...
		chartElements = new JComboBox<String>(); // added to the North of colPan
	...
	public void populateChartElements() {
		...
		if (colScheme.getNumOfAreas() == 1)
			chartElements.addItem("Area");
	}
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (elementChosen == "Axes")
			colScheme.setAxisColor(rd, gr, bl);
		else {
			if (elementChosen == "Chart Area")
				colScheme.setChartAreaColor(rd, gr, bl);
			else {
				if (elementChosen == "Font")
					colScheme.setFontColor(rd, gr, bl);
				else {
					if (elementChosen == "Area")
						colScheme.setAreaColor(rd, gr, bl);
					else {
