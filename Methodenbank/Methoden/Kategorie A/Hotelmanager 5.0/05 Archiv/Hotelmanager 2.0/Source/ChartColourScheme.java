...
{	private Color chartAreaColor;
	...
	barColors = new Vector();		// we will populate this with default colours
		for (int a = 0; a < 3; a++)
		{
                        barColors.add(Color.black);		// although the size of this array
  			barColors.add(Color.darkGray);		// is dynamic for convenience
			barColors.add(Color.gray);             // we make 18 entries - enough for most requirements.
	...
	lineColors = new Vector();		// similarly we will construct a large number for safety
		for (int b = 0; b < 3; b++)
		{
...
//	String elementChosen = chartElements.getSelectedItem().toString();
	...
	if (elementChosen == "Axes") colScheme.setAxisColor(rd, gr, bl);
		...
		{
		if (elementChosen == "Chart Area") colScheme.setChartAreaColor(rd, gr, bl);
			else
			{
