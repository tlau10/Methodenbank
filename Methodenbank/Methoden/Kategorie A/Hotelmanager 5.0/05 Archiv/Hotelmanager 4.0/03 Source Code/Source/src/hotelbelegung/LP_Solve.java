...
public class LP_Solve implements Solver {
	...
	@Override
	public double[] calcModel(Frame1 frame, String[][] lp_modell,
		...
		try {
			...
			writeBatOut.write(this.getLpSolvVerzeichnis() + "lp_solve -p <"
		...
		while (!erfolgreich && versuche <= 20) {
			...
			try {
				...
				try {
					// System.out.println(String.valueOf(in.readLine()));

					String uebergabe = null;

					System.out.println(in.readLine());
					uebergabe = in.readLine().substring(28).trim();

					double aDouble = Double.parseDouble(uebergabe);
					// System.out.println("zweite " +
					// in.readLine().substring(28).trim());
