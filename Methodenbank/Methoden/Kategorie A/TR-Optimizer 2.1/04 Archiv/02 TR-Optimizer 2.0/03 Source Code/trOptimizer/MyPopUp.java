...
/**
 * <p>Title: MyPopUp</p>
 * <p>Description: Diese Klasse stellt ein PopUp-Menü dar, das aufgerufen wird, bei Rechtklick auf ein Objekt in der Zeichnungsfläche.</p>
...
public class MyPopUp extends JPopupMenu implements ActionListener {
  ...
  public void actionPerformed( ActionEvent e ) {
    if( e.getActionCommand().equalsIgnoreCase( streckeZeichnen.getText() ) ) {
      ...
      // Daten eingeben
      if( objektId <= 999 ) { // popUp vom empfänger
  ...
  /**
       * Diese Methode zeichnet die Strecke von einem zum anderen Objekt, also z.B. von einem Produzenten zu einem Empfänger.
   * Es merkt sich dabei die Quell-Koordinate (also von wo aus die Strecke gezeichnet werden soll) und bekommt nur
   * noch die Ziel-Koordinate übergeben.
   *
  ...
  /**
   * Diese Methode wird vom streckenAdapter aufgerufen um bei mouseMoved temporäre linien zu zeichnen.
   * Der Benutzer sieht so ständig eine Linie mit seiner Maus mitbewegen.
   *
  ...
  /**
   * Diese Methode liefert die Id des ausgwählten Objekts zurück.
   *
   * @return
   */
