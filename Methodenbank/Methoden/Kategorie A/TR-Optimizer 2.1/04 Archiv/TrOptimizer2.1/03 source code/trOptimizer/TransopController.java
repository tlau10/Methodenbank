 ...
 * <p>Description: Diese Klasse ist die Controller-Klasse des TR-Optimizer und steuert die Abl�ufe der Anwendung.</p>
...
public class TransopController {
	...
	private int empfaengerId = 1; // empfaenger ids zwischen 1 und 999
	...
	private int popUpX, popUpY; // damit gespeichert wird, wo ein popUp
								// aufgerufen wurde --> um linien zu zeichnen
	...
	/**
	 * Der Konstruktor erwartet ein MainFrame-Objekt und initialisiert die
	 * Klassenattribute.
	 *
	 * @param m
	 *            aktuelles mainFrame-Objekt
	 */
	...
	/**
	 * Diese Methode legt ein neues ProduzentenObject an und speichert es in
	 * einem vector.
	 */
	public void addProduzent() {
		tmpProduzent = new Produzent(((Math.random() * 450) + 10),
				((Math.random() * 350) + 10), produzentId);
	...
	/**
	 * Diese Methode legt ein neues empfaengerObject an und speichert es in
	 * einem vector
	 */
	public void addEmpfaenger() {
		tmpEmpfaenger = new Empfaenger((int) ((Math.random() * 450) + 10),
				(int) ((Math.random() * 350) + 10), empfaengerId);
	...
	/**
	 * Diese Methode legt ein neues transporterObjekt an und speichert es in
	 * einem vector
	 */
	public void addTransporter() {
		transportPos -= 50;

		tmpTransporter = new Transporter(transportPos,
				transporterPane.getHeight() - 35, transporterId);
	...
	/**
	 * Diese Methode legt ein neues streckenObject an und speichert es in einem
	 * vector
	 *
	 * @param x
	 *            xKoordinate der Strecke auf dem zeichenFeld
	 * @param y
	 *            yKoordinate der Strecke auf dem zeichenFeld
	 * @param objektId
	 *            id der Strecke
	 *
	 */
	...
	/**
	 * Diese Methode ueberprueft ob sich an der Stelle auf die im
	 * transporterPane geklickt wurde ein transporterObjekt befindet
	 *
	 * @param x
	 *            xKoordinate des klicks
	 * @param y
	 *            yKoordinate des klicks
	 * @return boolean true falls der Transporter in der richtigen Position ist,
	 *         false sonst
	 */
	public boolean isTransporterInPos(double x, double y) {
		...
		for (int i = 0; i < transporter.size(); i++) {
			...
			if (x >= t.getX() && x <= (t.getX() + t.getBreite())
					&& y >= (t.getY() - 15)
					&& y <= ((t.getY() - 15) + t.getHoehe())) {
	...
	/**
	 *
	 * Diese Methode ueberprueft ob sich an der Stelle auf die geklickt wurde
	 * ein Objekt befindet.
	 *
	 * @param x
	 *            = X-Koordinate des Maus-Klicks
	 * @param y
	 *            = Y-Koordinate der Maus-Klicks
	...
	public boolean isInPos(double x, double y) {
		...
		for (int i = 0; i < empfaenger.size(); i++) {
			e = (Empfaenger) empfaenger.get(i);
			if (x >= e.getX() && x <= (e.getX() + e.getBreite())
					&& y >= e.getY() && y <= (e.getY() + e.getHoehe())) {
		...
		for (int i = 0; i < produzenten.size(); i++) {
			p = (Produzent) produzenten.get(i);
			if (x >= p.getX() && x <= (p.getX() + p.getBreite())
					&& y >= p.getY() && y <= (p.getY() + p.getHoehe())) {
		...
		for (int i = 0; i < strecken.size(); i++) {
			...
			// innerhalb des Wertebereiches pr�fen ob der Klick auf der Kante
			// lag

			steigung = ((double) s.getY1Koordinate() - (double) s
					.getY2Koordinate())
					/ ((double) s.getX1Koordinate() - (double) s
							.getX2Koordinate());
			konstante = (-1.0) * (steigung * (double) s.getX1Koordinate())
					+ (double) s.getY1Koordinate();
	...
	/**
	 * Diese Methode fuehrt das Verschieben eines objektes an eine andere Stelle
	 * in der Zeichenfl�che aus
	 *
	 * @param x
	 *            - Koordinate
	 * @param y
	...
	public void move(int x, int y) {
		...
		if (gewaehlteId <= 999) { // empf�nger gew�hlt
			...
			for (int i = 0; i < empfaenger.size(); i++) {
				...
				if (e.getId() == gewaehlteId) {
					...
					if (((Math.abs(x - e.getX())) > e.getBreite())
							|| ((Math.abs(y - e.getY())) > e.getHoehe())) {
						if ((x <= (zeichenPane.getWidth() - 15) && x >= (zeichenPane
								.getWidth() - zeichenPane.getWidth()))
								&& (y <= (zeichenPane.getHeight() - 20) && y >= (zeichenPane
										.getHeight() - zeichenPane.getHeight()))) { // (Gr��e
																					// Zeichenfl�che
																					// beachten!)
																					// damit
																					// objekte
							...
							// zugeh�rige Strecke bewegen
							for (int j = 0; j < strecken.size(); j++) {
								if (((Strecke) strecken.get(j)).getZiel() == gewaehlteId) {
									((Strecke) strecken.get(j))
											.setX1Koordinate(x + 10);
									((Strecke) strecken.get(j))
											.setY1Koordinate(y + 10);
								}
							}
						}
					}
		...
		} else if (gewaehlteId >= 1000 && gewaehlteId <= 1999) { // produzent
																	// gew�hlt

			for (int i = 0; i < produzenten.size(); i++) {
				p = (Produzent) produzenten.get(i);
				if (p.getId() == gewaehlteId) {
					if (((Math.abs(x - p.getX())) > p.getBreite())
							|| ((Math.abs(y - p.getY())) > p.getHoehe())) {
						if ((x <= (zeichenPane.getWidth() - 15) && x >= (zeichenPane
								.getWidth() - zeichenPane.getWidth()))
								&& (y <= (zeichenPane.getHeight() - 20) && y >= (zeichenPane
										.getHeight() - zeichenPane.getHeight()))) { // (Gr��e
							...
							for (int j = 0; j < strecken.size(); j++) {
								if (((Strecke) strecken.get(j)).getQuelle() == gewaehlteId) {
									((Strecke) strecken.get(j))
											.setX2Koordinate(x + 10);
									((Strecke) strecken.get(j))
											.setY2Koordinate(y + 10);
								}
							}
						}
					}
	...
	/**
	 * Diese Methode dient dazu, Objekte von der Zeichenfl�che zu l�schen
	 *
	 * @param id
	 *            des zu l�schenden Objekts
	 */
	public void removeObjekt(int id) {

		if (id <= 999) { // empf�nger gew�hlt
			for (int i = 0; i < empfaenger.size(); i++) {
				if (((Empfaenger) empfaenger.get(i)).getId() == id) {
					empfaenger.remove(i); // empfaenger loeschen
				}
			}
			// zugeh�rige Strecke l�schen
			for (int i = 0; i < strecken.size(); i++) {
			...
			zeichenPane.setStrecken(strecken);
			zeichenPane.setEmpfaenger(empfaenger);
		} else if (id >= 1000 && id <= 1999) { // produzent gew�hlt
			for (int i = 0; i < produzenten.size(); i++) {
				if (((Produzent) produzenten.get(i)).getId() == id) {
					produzenten.remove(i); // produzent l�schen
				}
			}
			// zugeh�rige Strecke l�schen
			...
			zeichenPane.setStrecken(strecken);
			zeichenPane.setProduzenten(produzenten);
		} else if (id >= 2000 && id <= 2999) { // transport gew�hlt
			for (int i = 0; i < transporter.size(); i++) {
				if (((Transporter) transporter.get(i)).getId() == id) {
					transporter.remove(i);
				}
			}
			// positionen der verbleibenden transporter neu setzen, damit sich
			...
			transportPos += 60;
			transporterPane.setTransporter(transporter);
		} else if (id >= 3000 && id <= 3999) { // strecke gew�hlt
			for (int i = 0; i < strecken.size(); i++) {
	...
	/**
	 * Diese Methode dient dazu, eine temporaere Linie zu zeichnen, so lange der
	 * Benutzer streckeZeichnen ausgew�hlt hat
	 *
	 * @param x2
	 *            xKoordinate der bewegung
	 * @param y2
	 *            yKoordinate der bewegung
	 * @param objektId
	 */
	public void printTempLine(int x2, int y2, int objektId) {
		zeichenPane.setPrintLine(); // damit die zeichenPane weiss, dass sie
									// jetzt eine linie zeichnen muss
		zeichenPane.setLineKoordinaten(popUpX - 10, popUpY - 10, x2, y2);
		mainFrame.repaint();
	}

	/**
	 * Diese Methode entscheidet ob die tempor�re Linie einen richtigen
	 * Endpunkt hat und dauerhaft angelegt werden soll
	 *
	 * @param x2
	 *            xKoordinate des klicks
	 * @param y2
	 *            yKoordinate des klicks
	 * @param objektId
	 *            des objektes, von wo aus die linie gezogen wurde
	 */
	public void printLine(int x2, int y2, int objektId) {

		boolean sollGezeichnetWerden = true;
		// der endpunkt der linie liegt auf einem objekt gewaehlteId ist die Id
		// dieses Objektes
		if (this.isInPos(x2, y2)) {
			...
			for (int i = 0; i < strecken.size(); i++) {
				...
				} else if (s.getQuelle() == gewaehlteId
						&& s.getZiel() == objektId) {
					sollGezeichnetWerden = false;
				}
			...
			if (sollGezeichnetWerden) {
				// anfangspunkt der Linie liegt auf einem empf�nger
				if (objektId <= 999) {
					...
					if (gewaehlteId >= 1000 && gewaehlteId <= 1999) {
						...
						for (int i = 0; i < produzenten.size(); i++) {
							p = (Produzent) produzenten.get(i);
							if (p.getId() == gewaehlteId) {
								this.addStrecke((int) (p.getX()),
										(int) (p.getY()), objektId);
							}
						}
					}

				} else if (objektId >= 1000 && objektId <= 1999) { // anfangspunkt
																	// der linie
																	// liegt auf
																	// einem
																	// produzenten
					if (gewaehlteId > 999) {
						mainFrame.repaint();
					} else { // ist der endpunkt ein empf�nger? dann zeichnen
						Empfaenger e;
			...
			zeichenPane.setStrecken(strecken); // neuen Vector zum zeichnen
												// �bergeben
			mainFrame.repaint();
	...
	/**
	 * Diese Methode dient dazu, um bei der Menuauswahl "neu", alle Objekte zu
	 * loeschen und ein leeres ZeichenFeld aufzubauen
	 */
	public void leereAlleVectoren() {
		...
		// alle ids wieder auf anfang zur�ck-setzen
	...
	/**
	 * Diese Methode dient dazu, um das Ergebnis bzw. die Einzelergebnisse einer
	 * Berechnung zu parsen und aufzubereiten. Das Ergebnis wird sp�ter in dem
	 * Textarea ausgeben.
	 *
	 * @param erg
	 *            ein Vector, der die Ergebnisse der Berechnung enth�lt
	...
	public String parseErg(Vector erg) {
		...
		for (int i = 0; i < erg.size(); i++) {
			// System.out.println( "Transporter: " + ( ( Ergebnis )erg.get( i )
			// ).getTransporter() );
			// System.out.println( "Ladung: " + ( ( Ergebnis )erg.get( i )
			// ).getLadung() );
			// System.out.println( "Quelle: " + ( ( Ergebnis )erg.get( i )
			// ).getQuelle() );
			// System.out.println( "Ziel: " + ( ( Ergebnis )erg.get( i )
			// ).getZiel() );
			// System.out.println( "Kosten: " + ( ( Ergebnis )erg.get( i )
			// ).getKosten() );
			/*
			 * if(( ( Ergebnis )erg.get( i ) ).getTransporter().length()==0) {
			 * ergebnis = "FEHLER: Transporter "+i+" hat keine Bezeichung!"; }
			 * 
			 * ergebnis += " Transporter "+( ( Ergebnis )erg.get( i )
			 * ).getTransporter(); ergebnis += " Ladung "+( ( Ergebnis )erg.get(
			 * i ) ).getLadung(); ergebnis += " Quelle "+( ( Ergebnis )erg.get(
			 * i ) ).getQuelle(); ergebnis += " Ziel "+( ( Ergebnis )erg.get( i
			 * ) ).getZiel(); ergebnis += " Kosten "+( ( Ergebnis )erg.get( i )
			 * ).getKosten();
			 */
		}
		System.out.println("\nErgebnis-Ende");
			...
			for (int i = 0; i < erg.size(); i++) {
				...
				if (tmpErgebnis.getKosten() != 0) {
					...
					for (int j = 0; j < empfaenger.size(); j++) {
						if (((Empfaenger) empfaenger.get(j)).getId() == ((Ergebnis) erg
								.get(i)).getZiel()) {
							tmpZielName = ((Empfaenger) empfaenger.get(j))
									.getName();
						}
					}
					// produzenten mit ergebnis-quelle vergleichen
					for (int j = 0; j < produzenten.size(); j++) {
						if (((Produzent) produzenten.get(j)).getId() == ((Ergebnis) erg
								.get(i)).getQuelle()) {
							tmpQuellenName = ((Produzent) produzenten.get(j))
									.getName();
						}
					}

					ergebnis += "    "
							+ ((Ergebnis) erg.get(i)).getTransporter()
							+ " faehrt mit Ladung ";
					ergebnis += ((Ergebnis) erg.get(i)).getLadung() + " von ";
					ergebnis += tmpQuellenName + " nach " + tmpZielName;
					ergebnis += " mit Kosten: "
							+ ((Ergebnis) erg.get(i)).getKosten() + "\n";

				}
			}
			if (GlobaleVariable.solvertyp == 1) {
				ergebnis += "\n";
				ergebnis += "    Solvertyp: LP Solve";
			} else if (GlobaleVariable.solvertyp == 2) {
				ergebnis += "\n";
				ergebnis += "    Solvertyp: Weidenauer";
			} else {
				ergebnis += "\n";
				ergebnis += "    Solvertyp: MOPS";
			}
		}
	...
	/**
	 * Diese Methode dient dazu, um das Gesamt-Ergebnis einer Berechnung zu
	 * parsen und aufzubereiten. Das Ergebnis wird sp�ter in dem Textfeld
	 * ausgeben.
	 *
	 * @param erg
	 *            ein Vector, der die Ergebnisse der Berechnung enth�lt
	...
	/**
	 * Diese Methode setzt das die Zeichenfl�che der Produzenten bzw.
	 * Empf�nger der Anwendung.
	 *
	 * @param z
	 *            aktuelles zeichenPane
	 */
	...
	/**
	 * Diese Methode setzt die Zeichenfl�che f�r die Transporter der
	 * Anwendung.
	 *
	 * @param t
	 *            aktuelles transporterPane
	 */
	...
	/**
	 * Diese Methode setzt die Id des gerade gew�hlten Objekts.
	 *
	 * @param id
	 *            des ausgew�hlten objektes
	 */
	...
	/**
	 * Diese Methode setzt die Empf�nger, die aktuell modelliert sind und sich
	 * auf der Zeichenfl�che befinden.
	 *
	 * @param _empfaenger
	 *            angelegte empfaenger werden an das zeichenPane uebergeben
	 *            (Datei oeffnen)
	 */
	...
	/**
	 * Diese Methode setzt die Produzenten, die aktuell modelliert sind und sich
	 * auf der Zeichenfl�che befinden.
	 *
	 * @param _produzenten
	 *            angelegte produzenten werden an das zeichenPane uebergeben
	 *            (Datei oeffnen)
	 */
	...
	/**
	 * Diese Methode setzt die Strecken, die aktuell modelliert sind und sich
	 * auf der Zeichenfl�che befinden.
	 *
	 * @param _strecken
	 *            angelegte strecken werden an das zeichenPane uebergeben (Datei
	 *            oeffnen)
	 */
	...
	/**
	 * Diese Methode setzt die Transporter, die aktuell modelliert sind und sich
	 * auf der Zeichenfl�che befinden.
	 *
	 * @param _transporter
	 *            angelegte transporter werden an das transporterPane uebergeben
	 *            (Datei oeffnen)
	 */
	...
	/**
	 * Diese Methode setzt die Id des Empf�ngers.
	 *
	 * @param id
	 *            des Empf�ngers
	 */
	...
	/**
	 * Diese Methode liefert die Id des gew�hlten Objekts zur�ck.
	 *
	 * @return id des gew�hlten Objekts
	 */
	...
	/**
	 * Diese Methode liefert den aktuellen MainFrame zur�ck
	 * 
	...
	/**
	 * Diese Methode liefert das aktuelle ZeichenPane zur�ck.
	 *
	...
	/**
	 * Diese Methode liefert die aktuelle Transporter-Zeichenfl�che zur�ck.
	 *
	 * @return die aktuelle Transporter-Zeichenfl�che
	 */
	...
	/**
	 * Diese Methode liefert die aktuellen Empf�nger zur�ck.
	 * 
	 * @return die aktuellen Empf�nger
	 */
	...
	/**
	 * Diese Methode liefert die aktuellen Produzenten zur�ck.
	 * 
	...
	/**
	 * Diese Methode liefert die aktuellen Transporter zur�ck.
	 * 
	...
	/**
	 * Diese Methode liefert die aktuellen Strecken zur�ck.
	 * 
