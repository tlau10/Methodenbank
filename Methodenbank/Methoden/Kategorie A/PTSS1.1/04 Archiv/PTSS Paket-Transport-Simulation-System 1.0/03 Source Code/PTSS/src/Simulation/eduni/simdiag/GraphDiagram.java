...
class Linedata {
  String name;
  Vector points = new Vector();// vector of points
  public Linedata(String n) { name = n; }
  public void add(double x, double y) {
    points.addElement(new DPoint(x,y));
  }
  public DPoint getDPoint(int i) { return (DPoint)points.elementAt(i); }
  ...
  public void draw(Graphics g, int i, GraphPanel p) {
    ...
    for (int j=0; j<points.size()-1; j++) {
      DPoint p1 = (DPoint)points.elementAt(j);
      DPoint p2 = (DPoint)points.elementAt(j+1);
...
class GraphPanel extends Panel {
  String xax="X",yax="Y";
  Vector data = new Vector(); // Vector of Linedata
  ...
  Linedata getLine(String n) {
    for (int i=0; i<data.size(); i++) {
      Linedata ld = (Linedata)data.elementAt(i);
