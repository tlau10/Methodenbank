...
class TreeDakin {
	...
	TreeDakin(matrix m, String solverpath) {// constructor der beim ersten
											// anlegen eines objektes auf diese
	...
	TreeDakin(matrix m, String s, int muell, TreeDakin _father) {// konstruktor
																	// der von
																	// objekten,
																	// die in
																	// dieser
	...
	public void calculate() {
		...
		/*
		 * formatieren der Werte von object zu float, mit benutzung der klasse
		 * Float
		 */
		...
		for (int i = 1; i < ergebnisVektor.size(); i++) {
			...
			if (first == 0
					&& !(tmpArray[i].floatValue() % 2 == 1.0 || tmpArray[i]
							.floatValue() % 2 == 0.0)) {
